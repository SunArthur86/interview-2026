...
feynman:
  ...
  - RRF算法实现多路结果融合
memory_points:
- 多路检索结合向量（语义）、关键词（精确）、图谱（关系）三通道。
- 用RRF倒数排名融合算法：Score = Σ 1/(k+rank)，统一多路排序。
- 优势：互补短板，精确编号查关键词，模糊概念查向量，关联查图谱。
- 核心口诀：向量关键词图谱并行，RRF融合分数，召回准确双提升。
frequency: high
---
...
**30 秒电梯演讲：** 多路召回融合策略解决语义与精度矛盾——打个比方，像专家小组查资料，懂语义的找概念，擅长记号的找编号，最后汇总意见

**展开框架：**
1. **多路检索结合向量** — 多路检索结合向量（语义）、关键词（精确）、图谱（关系）三通道。
2. **用RRF倒数排名** — 用RRF倒数排名融合算法：Score = Σ 1/(k+rank)，统一多路排序。
3. **优势** — 互补短板，精确编号查关键词，模糊概念查向量，关联查图谱。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡 | "采用多路检索而不是单一向量检索，30 秒讲清楚。" | 开场钩子 |
| 0:40 | 概念定义动画 | "一句话：多路召回融合策略解决语义与精度矛盾" | 核心定义 |
| 1:20 | 要点图解 | "多路检索结合向量（语义）、关键词（精确）、图谱（关系）三通道。" | 要点 |
| 2:00 | 用RRF倒数排名融合算法图解 | "Score = Σ 1/(k+rank)，统一多路排序。" | 用RRF倒数排名融合算法 |
| 2:40 | 优势图解 | "互补短板，精确编号查关键词，模糊概念查向量，关联查图谱。" | 优势 |
