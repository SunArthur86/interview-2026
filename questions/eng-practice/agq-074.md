...
**30 秒电梯演讲：** 从用户行为中挖掘改进信号，驱动产品迭代。——打个比方，像司机踩刹车或打方向盘一样，系统根据用户的反应（反馈）来调整行驶路线。

**展开框架：**
1. **收集** — 显式点赞/踩，隐式重试/转人工，定期问卷，保留 Session 上下文。
2. **处理** — 自动分类归因，分析 Top 10 问题，驱动知识库更新与 Prompt 调优。
3. **闭环** — 反馈数据入仓，LLM 辅助标注，形成“问题-优化-效果”闭环。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡 | "用户反馈怎么样，30 秒讲清楚。" | 开场钩子 |
| 0:30 | 概念定义动画 | "一句话：从用户行为中挖掘改进信号，驱动产品迭代。" | 核心定义 |
| 1:00 | 收集图解 | "显式点赞/踩，隐式重试/转人工，定期问卷，保留 Session 上下文。" | 收集 |
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:30<br/>从用户行为中挖掘改进信号"]:::concept
    N3["1:00<br/>收集"]:::deep
  end
  subgraph Outro["🎯 收尾"]
