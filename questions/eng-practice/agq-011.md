---
id: agq-011
difficulty: L2
category: eng-practice
subcategory: 企业面试问答
tags:
- IO
- Elasticsearch
feynman:
  essence: 在有限的上下文窗口内，动态管理历史对话与知识。
  analogy: 像写摘要，老对话太长就压缩成要点，只保留最近聊天的原文。
  first_principle: 如何在有限的短期记忆中保留最关键的信息？
  key_points:
  - 分层管理：将Prompt、历史、检索、工具分开
  - 滑动窗口：保留最近N轮完整对话
  - 自动摘要：过期对话压缩保留核心信息
  - 预算控制：严格按百分比分配Token配额
memory_points:
- 分层管理：System(10%)、History(30%)、RAG(40%)、Output(20%)分配Token预算。
- 窗口策略：保留最近N轮，超限触发异步摘要压缩，保留关键意图。
- 摘要逻辑：摘要保留用户意图、关键决策，而非简单截断。
- 实战痛点：长对话未压缩导致新问题被截断，需确保预算内优先保留新输入。
- 结果指标：多轮一致性提升至94%，支持50轮对话不丢失上下文。
frequency: high
---

# 怎么设计的对话管理和上下文维护

**Situation：** 企业客服场景中多轮对话非常普遍,需要在多轮交互中维护一致的上下文,同时控制 token 消耗.
**Task：** 设计对话管理系统,在有限的 context window 内高效利用上下文信息.
**Action：** 
1. **分层上下文管理**:
   - **系统层：** System Prompt(固定,约 800 tokens),定义 Agent 角色和行为规范.
   - **会话层：** 对话历史(动态,滑动窗口),最近 N 轮对话.
   - **检索层：** RAG 检索到的知识上下文(动态,按相关性排序).
   - **工具层：** 工具调用结果(临时,用完即清).

2. **窗口管理策略**:
   维护最近 10 轮对话的完整记录.
   超过 10 轮时,前面的对话自动摘要压缩.
   **摘要保留关键信息：** 用户意图、关键决策、未解决的问题.

3. **Token 预算分配**:
   假设总 context window 为 8K tokens:
   - System Prompt: 800 tokens (10%)
   - 对话历史 + 摘要: 2400 tokens (30%)
   - **检索上下文：** 3200 tokens (40%)
   - **预留输出：** 1600 tokens (20%)

**实战案例**：在长对话中，系统曾因未压缩历史，导致上下文超出 Token 限制截断了最新的用户问题。优化后，当历史 Token > 2000 时触发异步摘要，确保新问题始终在 Prompt 内。

**关键代码：**
```python
# 滑动窗口与摘要逻辑
MAX_HISTORY_TOKENS = 2000

def build_context(history):
    current_tokens = count_tokens(history)
    if current_tokens <= MAX_HISTORY_TOKENS:
        return history
    
    # 超出预算，保留最近 3 轮，其余摘要
    recent_msgs = history[-6:] # 3轮 = user+assistant * 3
    old_msgs = history[:-6]
    
    summary = llm_summary("Summarize key info:", old_msgs)
    
    return [{"role": "system", "content": f"History Summary: {summary}"}] + recent_msgs
```

**上下文构建与压缩流程图 (ASCII):**
```text
┌───────────────────────────────────────────────────────────┐
│                   Token Budget Manager (8K)                │
│  ┌────────────┐ ┌──────────────┐ ┌─────────────┐           │
│  │ System(10%)│ │ History(30%) │ │ Output(20%) │ (Fixed)   │
│  └────────────┘ └──────┬───────┘ └─────────────┘           │
└─────────────────────────┼─────────────────────────────────┘
                          │
                          ▼ (Space Left: 40%)
              ┌───────────────────────┐
              │  Dynamic Context Pool │
              └───────────┬───────────┘
                          │
        ┌─────────────────┼──────────────────┐
        ▼                 ▼                  ▼
  [Recent Msgs]   [Old Msg Summary]   [RAG Chunks]
        │                 │                  │
        └─────────────────┼──────────────────┘
                          │
                          ▼
              ┌───────────────────────┐
              │  Prompt Assembler     │
              └───────────┬───────────┘
                          │
                          ▼
                   LLM Inference
```

**Result：** 
- 多轮对话的上下文一致性从 76% 提升到 94%(基于人工评估).
- token 消耗控制在预算内,无超限导致的截断问题.
- 支持最长 50 轮对话不丢失关键上下文.

## 常见考点
- **摘要的信息丢失**：压缩后的摘要是否会丢失用户的个性化语气或细节？（回答：通常会保留事实性信息，可能丢失语气，但在摘要 Prompt 中可以要求保留关键风格）
- **滑动窗口 vs 摘要**：为什么选择摘要而不是仅仅丢弃最早的对话？（回答：摘要保留了早期的关键意图，而丢弃可能导致上下文断裂）


## 核心流程图

```mermaid
flowchart TD
    Start([🚀 客户端请求分布式系统]):::start
    Gateway[API 网关<br/>入口路由]:::process
    Coordinator[协调者 Coordinator<br/>2PC/3PC/TCC]:::process
    PhaseQ{{一致性协议?<br/>CAP 权衡}}:::decision
    CP[CP 强一致<br/>Raft/Paxos]:::process
    AP[AP 最终一致<br/>Gossip/Dynamo]:::process
    PartitionQ{{网络分区?<br/>Network Partition}}:::decision
    Minority[少数派降级<br/>拒绝服务]:::warn
    Majority[多数派继续<br/>保证一致]:::process
    Replica[多副本写入<br/>Leader → Followers]:::process
    QuorumQ{{Quorum 写策略?}}:::decision
    WQAll[W=All 同步所有副本<br/>强一致 低可用]:::process
    WQMajority[W=Majority<br/>平衡]:::process
    VectorClock[向量时钟<br/>冲突检测]:::process
    ConflictQ{{写冲突?}}:::decision
    LWW[Last Write Wins<br/>时间戳定序]:::process
    Merge[业务合并<br/>CRDT/手动解决]:::process
    Compensate[补偿事务 TCC<br/>Try-Confirm-Cancel]:::process
    MQ[(消息队列<br/>最终一致性)]:::store
    Final([✅ 全局一致状态]):::start

    Start --> Gateway --> Coordinator --> PhaseQ
    PhaseQ -->|强一致| CP --> PartitionQ
    PhaseQ -->|高可用| AP --> Replica
    PartitionQ -->|是| Minority
    PartitionQ -->|否| Majority --> Replica
    Replica --> QuorumQ
    QuorumQ -->|强一致| WQAll --> VectorClock
    QuorumQ -->|平衡| WQMajority --> VectorClock
    VectorClock --> ConflictQ
    ConflictQ -->|无冲突| MQ --> Final
    ConflictQ -->|有冲突| LWW --> Final
    ConflictQ -->|复杂| Merge --> Final
    PhaseQ -.跨服务.-> Compensate --> MQ

    classDef start fill:#2563eb,stroke:#1e3a8a,color:#fff,stroke-width:2px;
    classDef process fill:#dbeafe,stroke:#3b82f6,color:#1e3a8a;
    classDef decision fill:#fef3c7,stroke:#f59e0b,color:#78350f,stroke-width:2px;
    classDef store fill:#8b5cf6,stroke:#6d28d9,color:#fff;
    classDef warn fill:#fee2e2,stroke:#ef4444,color:#7f1d1d;

```

## 记忆要点

- 分层管理：System(10%)、History(30%)、RAG(40%)、Output(20%)分配Token预算。
- 窗口策略：保留最近N轮，超限触发异步摘要压缩，保留关键意图。
- 摘要逻辑：摘要保留用户意图、关键决策，而非简单截断。
- 实战痛点：长对话未压缩导致新问题被截断，需确保预算内优先保留新输入。
- 结果指标：多轮一致性提升至94%，支持50轮对话不丢失上下文。


## 结构化回答

**30 秒电梯演讲：** 在有限的上下文窗口内，动态管理历史对话与知识。——打个比方，像写摘要，老对话太长就压缩成要点，只保留最近聊天的原文。

**展开框架：**
1. **分层管理** — System(10%)、History(30%)、RAG(40%)、Output(20%)分配Token预算。
2. **窗口策略** — 保留最近N轮，超限触发异步摘要压缩，保留关键意图。
3. **摘要逻辑** — 摘要保留用户意图、关键决策，而非简单截断。

**收尾：** 以上三点都能配合实战聊。您想深入聊哪一块？

## 视频脚本

> 预计时长：2 分钟 | 由浅入深

| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡 | "怎么设计的对话管理和上下文维护，30 秒讲清楚。" | 开场钩子 |
| 0:30 | 概念定义动画 | "一句话：在有限的上下文窗口内，动态管理历史对话与知识。" | 核心定义 |
| 1:00 | 分层管理图解 | "System(10%)、History(30%)、RAG(40%)、Output(20%)分配Token预算。" | 分层管理 |
| 1:30 | 总结卡 | "记好这几条，面试不慌。下期见。" | 收尾 |

### 视频流程图

```mermaid
flowchart LR
  subgraph Intro["🎬 引入"]
    N1["0:00<br/>开场钩子"]:::open
  end
  subgraph Body["📚 讲解"]
    N2["0:30<br/>在有限的上下文窗口内"]:::concept
    N3["1:00<br/>分层管理"]:::deep
  end
  subgraph Outro["🎯 收尾"]
    N4["1:30<br/>收尾总结"]:::summary
  end
  N1 --> N2
  N2 --> N3
  N3 --> N4
  classDef open fill:#f59e0b,stroke:#b45309,color:#fff,stroke-width:2px;
  classDef concept fill:#3b82f6,stroke:#1e3a8a,color:#fff;
  classDef deep fill:#10b981,stroke:#047857,color:#fff;
  classDef practice fill:#8b5cf6,stroke:#6d28d9,color:#fff;
  classDef summary fill:#6b7280,stroke:#374151,color:#fff,stroke-width:2px;
```
