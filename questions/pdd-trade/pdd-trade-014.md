...
- 池化
feynman:
  essence: 线程池用"核心+队列+非核心+拒绝"四级水位应对流量，阿里规约禁 Executors（无界 OOM），必须 new ThreadPoolExecutor
    有界。
...
flowchart TD
    ...
    class K error
    class L info
    class LinkedBlockingQueue start
    class M process
    class br decision
    class corePool special
    class maxPool error
    class queue info
    class size start
    A["Tomcat线程<br/>提交下单任务"] --> B{"corePool<br/>满了吗?"}
    B -- "否" --> C["创建核心线程<br/>直接执行"]
    B -- "是" --> D{"任务队列<br/>满了吗?"}
    D -- "否" --> E["进入<br/>LinkedBlockingQueue"]
    D -- "是" --> F{"maxPool<br/>满了吗?"}
    F -- "否" --> G["创建非核心线程<br/>扩容执行"]
    F -- "是" --> H["触发CallerRunsPolicy<br/>Tomcat线程自己跑反压"]
    E --> I["空闲时被提取执行"]
    G --> I
    I --> J["执行完毕"]
    H --> J
