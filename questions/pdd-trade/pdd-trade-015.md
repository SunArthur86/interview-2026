...
flowchart TD
    ...
    class hash start
    class n process
    class newCap decision
    class oldCap special
    class put error
    class size info
    A["put(Key, Value)"] --> B["计算扰动 hash"]
    B --> C["数组定位 (n-1)&hash"]
    C --> D{"槽位为空?"}
    D -- 是 --> E["放入空桶"]
    D -- 否 --> F{"key相等?<br/>equals比较"}
    F -- 是 --> G["覆盖旧Value"]
    F -- 否 --> H{"链表长度 >= 8<br/>且数组 >= 64?"}
    H -- 否 --> I["尾插法挂入链表"]
    H -- 是 --> J["链表转为红黑树<br/>树化兜底"]
    E --> K{"size > capacity * 0.75?"}
    I --> K
    G --> L["更新完成"]
    K -- 是 --> M["触发扩容<br/>newCap = oldCap << 1"]
    K -- 否 --> L
    M --> N["按高位bit迁移数据"]
    N --> L
```
