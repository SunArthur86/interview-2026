...
flowchart TD
    ...
    class String error
    class ZINCRBY info
    class ZREVRANGE start
    class ZSet process
    class activity decision
    class cart special
    class daily error
    class distributed_lock info
    class like start
    class oid process
    class post decision
    class rank special
    ...
    class uid start
    class uv process
    A[交易系统业务请求] --> B{Redis 数据结构路由}
    B -->|Hash| C[cart:uid 购物车 KV映射]
    C --> D[执行 HINCRBY 原子加减商品数量]
    B -->|ZSet| E[rank:sales:sid 销量排行榜]
    E --> F[执行 ZINCRBY 更新销量并 ZREVRANGE 取Top榜]
    B -->|HyperLogLog| G[uv:activity:daily UV统计]
    G --> H[执行 PFADD 去重统计千万级UV]
    B -->|Set| I[like:post:uid 点赞去重集合]
    I --> J[执行 SADD 判断是否已点赞防重复]
    B -->|String| K[distributed_lock:oid 分布式锁]
    K --> L[执行 SET NX EX 防止订单重复处理]
```
