...
```mermaid
sequenceDiagram
    participant Order as 订单服务<br/>(调用方)
    participant Feign as Feign客户端
    participant LB as 负载均衡<br/>(LoadBalancer)
    participant Inv as 库存服务<br/>(提供方)
    participant Snt as 熔断降级<br/>(Sentinel)
    
    Order->>Feign: 1. 发起扣减库存RPC
    Feign->>Snt: 2. 校验熔断器状态(OPEN/CLOSED)
    Snt-->>Feign: 3. 状态放行
    Feign->>LB: 4. 路由选取健康实例
    LB-->>Feign: 5. 返回目标IP:Port
    Feign->>Inv: 6. 发起HTTP请求(带超时控制)
    
    alt 调用成功
        Inv-->>Feign: 7a. 正常响应
        Feign-->>Order: 8a. 返回结果
    else 超时或网络异常
        Inv-->>Feign: 7b. Read/Connect Timeout
        Feign->>Feign: 8b. 触发重试(读不写重试)
        Feign->>Snt: 9b. 记录错误率指标
        Snt-->>Feign: 10b. 触发熔断或执行降级
        Feign-->>Order: 11b. 执行Fallback返回默认/失败
    end
```
...
<img src="/interview-2026/images/diagram_pdd-trade_pdd-trade-011.svg" alt="【拼多多交易】Feign 怎么用？服务间调用怎么治理？ - 核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
