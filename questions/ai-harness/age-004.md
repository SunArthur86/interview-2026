...
flowchart LR
    ...
    subgraph Intro["🎥 引入"]
        N0["标题《语义缓存 vs 精确缓存》+…<br/>0:00"]:::intro
    end

    subgraph Core["📖 核心讲解"]
        N1["精确缓存：Hash 匹配 + 幂等场景<br/>0:25"]:::core
        N2["语义缓存：向量相似度 + 阈值权衡<br/>0:55"]:::deep
    end

    subgraph Practice["🔧 实战"]
        N3["风险控制：关键词校验 + 强合规禁用<br/>1:25"]:::practice
    end

    subgraph Wrap["🎬 收尾"]
        N4["成本考量：向量计算 + 存储成本<br/>1:50"]:::wrap
    end
