...
**30 秒电梯演讲：** 确定量化映射的截断范围以最小化信息损失——打个比方，像拍照调光圈，MinMax怕过曝（异常值），Percentile则裁掉极亮背景

**展开框架：**
1. **MinMax** — 取绝对最大值定范围。对Outlier敏感，易导致正常数据压缩，适合权重。
2. **Percenti** — Percentile：取分位数(如99.9%)截断Outlier。牺牲极值保护主流分布，适合激活值。
3. **其他算法** — KL散度最小化分布差异(信息论)，MSE最小化数值误差，ACIQ解析解。

**收尾：** 以上三点都能配合实战聊。我可以展开任一要点，比如「为什么不用 mean±3σ？—— 大模型激活不一定是高斯分布，可能有重尾」这类追问您感兴趣吗？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡 | "【智谱Infra面经】简述 MinMax 和 Percentile 校准算法有什，30 秒讲清楚。" | 开场钩子 |
| 0:40 | 概念定义动画 | "一句话：确定量化映射的截断范围以最小化信息损失" | 核心定义 |
| 1:20 | MinMax图解 | "取绝对最大值定范围。对Outlier敏感，易导致正常数据压缩，适合权重。" | MinMax |
