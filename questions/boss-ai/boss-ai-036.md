...
- 结构化输出
feynman:
  essence: Prompt 工程进阶是用 CoT（思维链）/ReAct（推理+行动）/Few-shot（范例）/结构化输出等技巧，把'写自然语言问 LLM'升级为'用结构化策略引导
    LLM 高质量推理和输出'。
...
- ReAct 和 Tool Calling 啥关系？——ReAct 是'推理+行动'范式（Thought/Action/Observation 循环）；Tool
  Calling 是工程实现；ReAct 用 Tool Calling 落地。
- 结构化输出怎么保证 JSON 不挂？——强 prompt 指令 + JSON Schema 约束 + grammar-constrained decoding（vLLM/outlines）+
  解析失败重试。
memory_points:
- CoT：先推理再答
...
flowchart TD
    ...
    class Prompt info
    class ReAct start
    class Schema process
    class br decision
    class shot special
    A["用户复杂提问"] --> B["Few-shot 范例注入"]
    B --> C["Prompt 引导<br/>LLM 推理引擎"]
    C --> D["CoT 思维链<br/>拆解步骤"]
    D --> E["ReAct 循环<br/>推理与行动"]
    E --> F{"信息是否充足?"}
    F -- 否 --> G["Action<br/>调用外部工具"]
    G --> H["Observation<br/>获取工具返回结果"]
    H --> E
    F -- 是 --> I["合并推理结论"]
    I --> J["结构化输出约束<br/>JSON Schema"]
    J --> K["稳定可解析响应"]
```
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:15<br/>Prompt 模板代码截"]:::concept
    N3["0:40<br/>ReAct"]:::concept
    N4["1:05<br/>Few-shot"]:::deep
  end
  subgraph Outro["🎯 收尾"]
