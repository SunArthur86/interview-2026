...
feynman:
  ...
  first_principle: 不同 LLM 厂商 API 协议、参数、能力、计费各不相同，业务直接对接 = 强耦合、难切换、难统一治理。必须抽象统一 SDK
    层，让上层只面对标准接口，下层适配各家。
  key_points:
...
follow_up:
- 工具调用接口怎么统一？——OpenAI function calling、Claude tools、通义 plugins 协议不同；统一抽象成 tools 字段，adapter
  做协议转换。
