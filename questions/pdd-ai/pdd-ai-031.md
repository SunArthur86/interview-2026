...
- 中台
feynman:
  essence: 用 AI Agent 改造中台是"把 LLM 当大脑、中台能力当工具，让 Agent 自主调用特征/规则/模型/实验完成复杂任务"，从"人调
    API"升级为"Agent 自主演绎"。
...
flowchart TD
    ...
    subgraph 触发与输入
        A1[用户开放式提问] --> B[LLM Agent 大脑]
    end
    subgraph Agent 编排循环 ReAct 模式
        B --> C1[Thought 思考推理]
        C1 --> C2[Action 选择工具]
        C2 --> C3[Observation 观察结果]
        C3 -- 继续执行 --> C1
    end
    subgraph 中台能力工具化
        D1[工具: 查询订单状态]
    ...
    C2 -- Function Calling JSON --> D1
    C2 -- Function Calling JSON --> D2
    C2 -- Function Calling JSON --> D3
    C2 -- Function Calling JSON --> D4
    D1 --> E[返回结构化执行结果]
    D2 --> E
    D3 --> E
    D4 --> E
    E --> C3
    C3 -- 任务完成 --> F[输出最终自然语言回答]
```
