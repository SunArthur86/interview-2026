...
- 分布式训练
feynman:
  essence: 分布式训练多 GPU 数据并行时，AllReduce 是"同步各 GPU 梯度"的核心通信原语，Ring AllReduce 把通信量从 O(N)
    降到 O(1)，是百亿-千亿模型训练的命脉。
  analogy: 像 N 个人每人看一本书然后交换心得——朴素做法是每人挨个问（O(N²) 通信），Ring 做法是大家围一圈每人只和左右邻居传话，N-1 轮后人人全知道（通信量
    O(N·size)，带宽最优）。
...
sequenceDiagram
    ...
    class All start
    class AllReduce process
    class GPU0 decision
    class GPU1 special
    class GPU2 error
    class GPU3 info
    class GPU4 start
    class Gather process
    class NVLink decision
    class Reduce special
    class Ring error
    class SGD info
    class Scatter start
    class Top process
    ...
    class k error
    class rgb info
    participant GPU1 as GPU0(NVLink)
    participant GPU2 as GPU1(NVLink)
    participant GPU3 as GPU2(IB网络)
    participant GPU4 as GPU3(IB网络)

    rect rgb(240, 248, 255)
    Note over GPU1, GPU4: Ring AllReduce 阶段一: Reduce-Scatter
    GPU1->>GPU2: 发送分块0并聚合
    GPU2->>GPU3: 发送分块1并聚合
    GPU3->>GPU4: 发送分块2并聚合
    end
    ...
    Note over GPU1, GPU4: Ring AllReduce 阶段二: All-Gather
    GPU4->>GPU1: 广播完整聚合块0
    GPU1->>GPU2: 广播完整聚合块1
    GPU2->>GPU3: 广播完整聚合块2
    end

    Note over GPU1, GPU4: 梯度压缩选项
    GPU1-->>GPU2: 1-bit SGD (符号量化)
    GPU2-->>GPU3: Top-k 稀疏化 (仅传大梯度)
```
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:12<br/>概念"]:::concept
    N3["0:51<br/>能力/参数"]:::deep
    N4["1:30<br/>通信量与 GPU 数无关"]:::deep
