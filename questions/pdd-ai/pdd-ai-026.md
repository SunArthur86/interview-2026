...
- LLM 推理池化
feynman:
  essence: 高并发"四件套"是池化（预建资源复用）+ 缓存（少打 DB）+ 扩容（弹性加机器）+ 异步队列（削峰），LLM 推理场景四者都不可或缺且要按
    GPU 特性调整。
...
flowchart TD
    ...
    A["高并发请求"] --> B["异步队列削峰 Kafka"]
    B --> C["网关与缓存层"]
    C --> D{"命中缓存?"}
    D -->|是| E["直接返回结果"]
    D -->|否| F["推理服务实例池"]
    subgraph F["GPU推理池化隔离"]
        F1["Worker 1"]
        F2["Worker 2"]
    end
    F --> G["GPU算力节点"]
    H["K8s HPA弹性扩容"] -.->|队列长度监控| F
    F --> I["输出结果"]
    I --> E
```
