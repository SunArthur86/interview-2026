...
```mermaid
flowchart TD
    subgraph Traditional[传统连续分配]
        A1[预分配最大长度KV Cache] --> A2[内部碎片浪费显存]
        A1 --> A3[外部碎片难以复用]
        A2 --> A4[显存利用率仅 20-40%]
    end
    subgraph VLLM[vLLM PagedAttention]
        B1[OS虚拟内存分页机制] --> B2[Block物理块池化]
        B3[请求逻辑页表] -->|按需申请| B2
        B2 --> B4[显存利用率达 90%+]
    end
    subgraph Optimize[推理吞吐优化]
        C1[Continuous Batching 连续批处理] --> C2[解决GPU空等]
        C3[Prefix Caching 前缀缓存] --> C4[解决重复计算]
    end
    Traditional -.->|管理方式升级| VLLM
    VLLM --> Optimize
```
...
<img src="/interview-2026/images/diagram_pdd-ai_pdd-ai-014.svg" alt="【拼多多 AI 中台】vLLM 和 PagedAttention 原理是什么？ - 核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
