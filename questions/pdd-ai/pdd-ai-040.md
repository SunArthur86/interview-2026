...
```mermaid
flowchart TD
    subgraph L [学习路径 6-12个月]
        A1[M1-M2 LLM基础与API调用] --> A2[M3-M4 RAG与Agent开发]
        A2 --> A3[M5-M6 推理工程与压测]
        A3 --> A4[M7-M8 训练框架与显存优化]
        A4 --> A5[M9-M10 分布式AI集群调度]
        A5 --> A6[M11-M12 业务线端到端实战]
    end

    subgraph J [Java 能力复用]
        J1(高并发系统架构设计)
        J2(微服务治理与K8s调度)
        J3(链路监控与性能调优)
    end

    subgraph P [Python 核心栈]
        P1(PyTorch 模型加载与推理)
        P2(vLLM/TensorRT 推理引擎)
    end
...
<img src="/interview-2026/images/diagram_pdd-ai_pdd-ai-040.svg" alt="【拼多多 AI 中台】Java 工程师怎么转大模型工程方向？ - 核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
