...
- 学习路径
feynman:
  essence: Java 工程师转大模型工程是"在已有工程能力上叠加 LLM 工程层"——不用从零学算法，学 LLM 应用（Prompt/RAG/Agent）+
    推理优化（vLLM/KV Cache/量化）+ Infra（GPU/RDMA），把"写业务"升级为"做 LLM 基础设施"。
