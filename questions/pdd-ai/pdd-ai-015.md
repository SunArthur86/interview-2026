...
flowchart TD
    subgraph Prefill集群
        A["Prompt输入"] --> B["计算Q/K/V<br/>算力瓶颈"]
        B --> C["生成完整KV Cache"]
    end

    C -->|"KV Cache传输"| D["显存池化复用"]

    subgraph Decode集群
        D --> E["读取历史KV Cache<br/>显存带宽瓶颈"]
        E --> F["增量计算新Token Q/K/V"]
        F --> G["追加更新KV Cache"]
        G --> E
    end

    H["KV Cache优化方案"] --> I["GQA/MQA减少头数"]
    H --> J["KV量化压缩"]
    H --> K["PagedAttention分页管理"]
    H --> L["Prefix Caching前缀复用"]
    I -.作用于.-> C
    K -.作用于.-> D
```
...
<img src="/interview-2026/images/diagram_pdd-ai_pdd-ai-015.svg" alt="【拼多多 AI 中台】KV Cache 原理是什么？怎么估算大小？ - 核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
