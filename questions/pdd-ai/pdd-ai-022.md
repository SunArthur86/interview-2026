...
flowchart TB
    ...
    vllm["vLLM 集群<br/>LLM 通用"]
    trtllm["TRT-LLM 集群<br/>LLM 极致"]
    sched["调度层（K8s + GPU Operator）<br/>Pod 调度 · 弹性扩缩（HPA/VPA） · 资源池化"]
    cross["横切：模型注册中心（MLflow） · 监控（Prometheus） · 日志（ELK） · 配置（Nacos）"]

    biz -->|HTTP/gRPC| gateway
    gateway --> triton
    gateway --> vllm
    gateway --> trtllm
    triton --> sched
