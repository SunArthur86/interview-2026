...
flowchart TB
    ...
    Network["网络层：<br/>- 机内：NVLink/NVSwitch（900 GB/s）<br/>- 机间：InfiniBand NDR（400 Gbps）/RoCE<br/>- GPU Direct RDMA（网卡直读 GPU）"]
    Storage["存储层：<br/>- 并行文件系统（Lustre/CephFS，TB/s 吞吐）<br/>- 对象存储（S3 兼容，海量冷数据）<br/>- 缓存（本地 SSD/NVMe）"]
    App --> Sched
    Sched --> Compute
    Compute --> Network
    Network --> Storage
...
<img src="/interview-2026/images/diagram_pdd-ai_pdd-ai-037.svg" alt="【拼多多 AI 中台】AI Infra（GPU 集群/RDMA/NVLink）怎么构建？ - 核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
