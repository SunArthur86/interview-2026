...
flowchart TD
    ...
    class Annotations start
    class Bean process
    class BeanDefinition decision
    class BeanDefinitionRegistry special
    class Config error
    class Java info
    class Map start
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["Spring IOC原理是什么？"]:::intro
    end

    subgraph Core["讲解"]
        B["核心定义：控制反转，将对象创建和依赖关系交给容器管理…"]:::core
        C["流程四步：资源定位 -》 解析为BeanDefini…"]:::deep
    end
