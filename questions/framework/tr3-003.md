...
flowchart TD
    ...
    class M info
    class Spring start
    class ThreadLocal process
    class Transactional decision
    class br special
    class flatMap error
    class synchronized info
    class unmount start
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["在Spring Boot 3.3中使用虚拟线程作为Web容器…"]:::intro
    end

    subgraph Core["讲解"]
        B["风险一：传统事务基于ThreadLocal，因为虚拟…"]:::core
        C["风险二：因为遇到synchronized或JNI会发…"]:::deep
    end
