...
flowchart TD
    ...
    class K start
    class L process
    class OP_ACCEPT decision
    class OP_READ special
    class OP_WRITE error
    class Pipeline info
    class Selector start
    class SocketChannel process
    class TCP decision
    class Worker special
    class WorkerGroup error
    class br info
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["Netty 的 Reactor 线程模型中，BossGrou…"]:::intro
    end

    subgraph Core["讲解"]
        B["职责对比：BossGroup专职处理TCP连接建立，…"]:::core
        C["线程配比：Boss通常仅1个线程，而Worker默认…"]:::deep
    end
