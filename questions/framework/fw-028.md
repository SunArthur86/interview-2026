...
flowchart TB
    ...
    class AOP start
    class After process
    class Aware decision
    class Bean special
    class BeanFactoryAware error
    class BeanNameAware info
    class BeanPostProcessor start
    class Before process
    class Init decision
    class InitializingBean special
    class S1 error
    class S2 info
    class S3 start
    class S4 process
    ...
    class Title special
    class afterPropertiesSet error
    class br info
    class etc start
    class init process
    class method decision
    class postProcessAfterInit special
    class postProcessBeforeInit error
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["Spring Bean 生命周期是什么？"]:::intro
    end

    subgraph Core["讲解"]
        B["生命周期四阶段：实例化、属性赋值、初始化、销毁"]:::core
        C["Aware接口回调：在初始化前注入底层容器组件"]:::deep
    end
