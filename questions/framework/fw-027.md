...
graph TD
    ...
    class Lookup error
    class Service info
    class SimpleDateFormat start
    class Spring process
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["Spring Bean 作用域是什么？"]:::intro
    end

    subgraph Core["讲解"]
        B["核心作用域：singleton单例（默认共享唯一）与…"]:::core
        C["Web三大作用域：request（单次HTTP请求）…"]:::deep
    end
