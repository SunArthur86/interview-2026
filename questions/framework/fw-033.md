...
flowchart TD
    ...
    class H process
    class I decision
    class NESTED special
    class READ_COMMITTED error
    class REPEATABLE_READ info
    class REQUIRED start
    class REQUIRES_NEW process
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["Spring事务的传播机制和隔离级别是什么？"]:::intro
    end

    subgraph Core["讲解"]
        B["传播控范围（默认REQUIRED同生共死），隔离防读…"]:::core
        C["三大常用传播：REQUIRED融入、REQUIRES…"]:::deep
    end
