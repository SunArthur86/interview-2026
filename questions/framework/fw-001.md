...
graph TD
    ...
    class Configuration info
    class Controller start
    class D process
    class D1 decision
    class D2 special
    class D3 error
    class Qualifier info
    class Repository start
    class Resource process
    class RestController decision
    class Service special
    class br error
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["Spring中常用的注解有哪些？"]:::intro
    end

    subgraph Core["讲解"]
        B["因为@Autowired默认按类型（ByType）注…"]:::core
        C["对比记忆：@Autowired是Spring特有按类…"]:::deep
    end
