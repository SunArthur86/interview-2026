...
flowchart LR
    ...
    class B4 decision
    class C special
    class EpollEventLoopGroup error
    class N info
    class NioEventLoopGroup start
    class O process
    class SO_REUSEPORT decision
    class br special
    class sendfile error
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["在构建高并发网关或服务时，Netty 的 `EpollEve…"]:::intro
    end

    subgraph Core["讲解"]
        B["底层数据结构：epoll用红黑树管理FD，而sele…"]:::core
        C["零拷贝优势：Epoll底层调用sendfile实现零…"]:::deep
    end
