...
flowchart TD
    ...
    class O info
    class OOM start
    class PhantomReference process
    class ReferenceQueue decision
    class SoftReference special
    class StrongReference error
    class ThreadLocalMap info
    class WeakHashMap start
    class WeakReference process
    class br decision
    class new special
...
flowchart LR
    ...
    subgraph Core["讲解"]
        B["强引用不回收：最普遍引用，宁可抛OOM异常也绝不回收…"]:::core
        C["软引用看内存：内存不足即将OOM时才回收，常用于网页…"]:::deep
    end
