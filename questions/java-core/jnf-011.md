...
flowchart TD
    ...
    class K process
    class L decision
    class Lombok special
    class M error
    class N info
    class O start
    class P process
    class Point decision
    class Q special
    class R error
    class Records info
    class S start
    ...
    class final error
    class get info
    class hashCode start
    class int process
    class record decision
    class toString special
    class x error
    class y info
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["什么是Record Patterns（记录模式）？它如何实现…"]:::intro
    end

    subgraph Core["讲解"]
        B["Java 21正式特性，核心价值是像函数式语言一样优…"]:::core
        C["支持嵌套解构，底层能自动推导var类型，且类型不匹配…"]:::deep
    end
