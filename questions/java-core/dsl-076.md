...
flowchart TD
    ...
    class UPDATE info
    class Unmount start
    class componentDidMount process
    class componentDidUpdate decision
    class componentWillUnmount special
    class constructor error
    class false info
    class getDerivedStateFromProps start
    class getSnapshotBeforeUpdate process
    class prevProps decision
    class prevState special
    class render error
    class setState info
    class shouldComponentUpdate start
...
flowchart LR
    ...
    subgraph Core["讲解"]
        B["三阶段速记：挂载、更新、卸载，每个阶段对应特定生命周…"]:::core
        C["挂载核心：constructor初始化 -》 ren…"]:::deep
    end
