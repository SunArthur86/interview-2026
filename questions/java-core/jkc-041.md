...
flowchart TD
    ...
    class A start
    class B process
    class BlockCache decision
    class BloomFilter special
    class C error
    class Column info
    ...
    class L decision
    class M special
    class MemStore error
    class N info
    class O start
    class P process
    class Q decision
    class Qualifier special
    class R error
    class Region info
    class RegionServer start
    class RowKey process
    class S decision
    class Store special
    class T error
...
flowchart LR
    ...
    subgraph Core["讲解"]
        B["核心模型：多维映射（RowKey, CF:Colum…"]:::core
        C["写入极快：因为采用LSM-Tree架构，先写WAL与…"]:::deep
    end
