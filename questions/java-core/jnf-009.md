...
flowchart TD
    ...
    class OS special
    class RPC error
    class ThreadLocal info
    class br start
    class monitor process
    class synchronized decision
    class unmount special
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["什么情况下虚拟线程不适用？使用虚拟线程时有哪些常见的陷阱和最…"]:::intro
    end

    subgraph Core["讲解"]
        B["场景不适：因为不阻塞不释放载体，所以CPU密集型任务…"]:::core
        C["线程固定陷阱：因为在synchronized内执行I…"]:::deep
    end
