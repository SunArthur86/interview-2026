...
```mermaid
flowchart TD
    A["事务发起SELECT查询"] --> B["获取记录的隐藏列DB_TRX_ID"]
    B --> C{"DB_TRX_ID == ReadView.creator_trx_id?"}
    C -- 是 --> D["可见：返回当前版本"]
    C -- 否 --> E{"DB_TRX_ID < min_trx_id?"}
    E -- 是 --> D
    E -- 否 --> F{"DB_TRX_ID >= max_trx_id?"}
    F -- 是 --> G["不可见：沿DB_ROLL_PTR查找"]
    F -- 否 --> H{"在活跃事务列表m_ids中?"}
    H -- 是 --> G
    H -- 否 --> D
    G --> I["获取Undo Log上一历史版本"]
    I --> B
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    ...
    N3["0:40<br/>read"]:::deep
    N4["1:05<br/>select"]:::deep
  end
  subgraph Outro["🎯 收尾"]
