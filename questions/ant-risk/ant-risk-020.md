...
flowchart TD
    ...
    B --> C[生成全局 Trace 上下文]
    C --> D[决策引擎评估业务逻辑]
    D --> E[RPC 调用: 特征服务获取特征 Span]
    D --> F[中间件 Span: Redis/MySQL/Kafka]
    E --> G{Span 是否存在子节点?}
    G -- 无子节点 --> H[排查: 服务未装Agent或插件缺失]
    G -- 有子节点 --> I[查看完整调用树定位耗时根源]
    F --> I
    I --> J[关联 Metrics 指标与 Logs 日志]
    J --> K[输出: P99 告警/慢查询根因分析]
```
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    ...
    N3["0:40<br/>Metrics 三种"]:::deep
    N4["1:05<br/>Trace 三个概念"]:::concept
  end
  subgraph Outro["🎯 收尾"]
