...
```mermaid
flowchart TD
    A["线程调用lock()"] --> B{"state=CAS更新为1<br/>竞争成功?"}
    B -- 是 --> C["获取锁成功<br/>执行业务逻辑"]
    B -- 否 --> D["封装Node加入<br/>CLH双向队列"]
    D --> E{"是公平锁?<br/>检查前驱节点"}
    E -- 是且有前驱 --> F["LockSupport.park()<br/>线程阻塞挂起"]
    E -- 否 --> G["自旋尝试获取"]
    G --> B
    C --> H["unlock()<br/>state设为0"]
    H --> I["唤醒Head后继Node"]
    I --> F
    F -- unpark唤醒 --> G
