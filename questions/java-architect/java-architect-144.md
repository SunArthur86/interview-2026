...
flowchart TD
    ...
    B --> C["本地批量预扣令牌"]
    C -- "本地余量充足" --> H["放行请求"]
    C -- "本地余量不足" --> D["Redis+Lua<br/>分布式令牌桶"]
    subgraph Redis全局存储 ["Redis全局存储"]
        D
        D -- "按维度分配" --> E["商家维度"]
        D -- "按维度分配" --> F["API维度"]
        D -- "按维度分配" --> G["IP维度"]
    ...
    D -- "获取成功" --> H
    D -- "超出配额/无令牌" --> I["拒绝: HTTP 429"]
    I --> J["响应Header<br/>附带Retry-After"]
    D -. "Redis主从切换延迟" .-> K["容忍5%-10%误差"]
```
...
<img src="/interview-2026/images/diagram_java-architect_java-architect-144.svg" alt="核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
