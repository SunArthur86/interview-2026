...
- 回滚
feynman:
  essence: 模型评测集、灰度与回滚体系的本质是"用数据证明新模型比旧模型好，用灰度控制风险，用回滚兜底故障"。评测集是"标尺"（离线量化模型质量），灰度是"探针"（小流量在线验证），回滚是"保险"（出问题秒级撤退）。三者构成
    AI 模型安全上线的闭环。
  analogy: 像新药上市——评测集是"临床试验"（离线验证有效性和安全性），灰度是"分批上市"（先小人群再全人群），回滚是"召回机制"（发现副作用立即下架）。
  first_principle: LLM 升级不像传统代码升级（确定性、可单测），它可能在某些场景变好、某些场景变差（能力分布变化）。必须用标注评测集离线对比、用灰度在线
    A/B、用回滚兜底，三者缺一不可。
  key_points:
...
first_principle:
  ...
  - 全量上线一旦出问题影响所有用户
  - 必须有快速回滚能力（不能等修 bug）
  rebuild: 三段式——(1) 离线评测：新模型跑标注评测集，对比旧模型的 accuracy/faithfulness/safety，不通过阻断；(2) 灰度发布：1%
    → 5% → 25% → 100%，每档监控核心指标 3 天，退化回滚；(3) 回滚兜底：版本化部署，流量切换秒级回滚。
follow_up:
...
- 指标退化多少回滚？——硬指标（safety violation、accuracy）任何退化立即回滚；软指标（helpfulness、latency）退化超 5%
  回滚。要有自动回滚机制不靠人判断。
memory_points:
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:15<br/>像新药上市评测集是临床试验（离线"]:::concept
    N3["0:40<br/>评测集分层要点"]:::deep
    N4["1:05<br/>评测指标要点"]:::deep
    N5["1:30<br/>灰度策略要点"]:::deep
  end
  subgraph Outro["🎯 收尾"]
...
|----------|------------------|--------------|
| 目标追问 | 传统代码上线有单测+灰度就够了，LLM 为什么非要搞评测集？ | LLM 升级效果非单调——A 场景变好 B 场景可能变差（能力分布迁移）。单测覆盖不了概率输出。必须用标注评测集分场景量化 accuracy/faithfulness，门禁不通过阻断。这是 LLM 上线和代码上线的根本差异 |
| 证据追问 | 你怎么证明评测集本身是准的？会不会评测集脏导致归因错？ | 三层校验：(1) 评测集从真实日志采样（不是合成）；(2) 10% 样本人工抽检 LLM-as-judge 的评分，agreement_rate > 85%；(3) 线上灰度时同步跑评测集，对比离线分和线上指标的相关性。监控 eval_online_gap（应 < 5%）|
| 边界追问 | 评测集和线上 gap 多大可接受？什么时候必须信线上不信评测？ | 软指标（helpfulness）gap 大可接受（主观），硬指标（safety_violation/complaint_rate）线上退化立即回滚不信评测。safety 是红线——离线评测说安全但线上有 1 起违规也要回滚 |
| 反例追问 | 给一个离线评测通过但线上翻车的真实反例？ | 评测集是"标准 query"，新模型在标准 query 上 accuracy +5%。但线上长尾 query（方言/错别字/超短）退化 20%，用户投诉率涨 30%。根因：评测集没覆盖长尾。修复：评测集要分层采样（高频 50% + 长尾 30% + 事故 10% + 边界 10%）|
| 风险追问 | 自动回滚会不会误判？比如大促流量突增导致投诉率自然涨，被当成模型问题回滚了？ | 自动回滚要排除流量因素——归一化投诉率（投诉/DAU 而非绝对值），且对比的是"同时段新旧模型的相对值"（new_complaint_rate / old_complaint_rate > 1.1 才回滚），不是绝对阈值。大促时新旧模型都涨就不触发回滚。监控 rollback_false_positive_rate |
| 验证追问 | 灰度 1% 流量跑 3 天够吗？样本量怎么算？ | power analysis 算最小样本——检测 1% 提升需要百万级请求，检测 5% 提升十万级够。1% 流量小业务 3 天可能不够。大流量业务（日百万）3 天够。关键是显著性检验——p-value 不显著就继续跑，不能拍脑袋说"够了" |
| 沉淀追问 | 多个 LLM 业务都要上线，你怎么避免每业务各搞一套评测灰度？ | 沉淀模型上线平台——评测集模板（通用层共享 + 业务层自填）、灰度阶梯标准（1→5→25→100）、自动门禁规则（safety 任何退化阻断）。提供 rollback_sla（秒级回滚能力）共享。监控按业务拆 srm_violation_rate 和 rollback_count 定位弱业务 |

### 现场对话示例

**面试官**：你说评测集要从日志采样，但日志里没有标准答案，怎么标？

**候选人**：分两步。LLM 辅助生成 reference answer（用强模型 GPT-4 生成标准答案），人工抽检 20% 校正。对于事实类 query（订单状态）能从数据库取 ground truth；对于开放类 query（退货建议）用"最佳历史回答"（用户满意度高的）作 reference。评测集不是一次性建完，每月补充新 case（从投诉日志采）。

**面试官**：LLM-as-judge 用 GPT-4 评 GPT-3.5，但如果未来我们升级到 GPT-4，judge 还用 GPT-4 评自己，自评会有偏好怎么办？

**候选人**：自评偏差是真实问题（模型倾向给自己高分）。对策：judge 用比被评测强一代的模型（评 GPT-4 用 GPT-4.5 或 Claude-3-Opus 交叉评）；或多个 judge 投票降方差；或关键场景人工复核（抽 10% 算 agreement_rate）。监控 judge_self_bias（自评 vs 他评的差值），超阈值换 judge。

**面试官**：灰度时新旧模型并行跑，但用户两次请求可能命中不同模型，体验不一致（第一次答 A 第二次答 B），怎么办？

**候选人**：用户 hash 分桶保证同一用户始终命中同一模型，不会串。这是灰度的硬约束——userId hash 到 [0,100)，灰度档位 5% 就让 bucket < 5 的用户全走新模型，其他人全走老模型。同一用户的多次请求都在同一桶，体验一致。监控 cross_model_session_rate（应为 0）。
