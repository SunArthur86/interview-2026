...
flowchart TD
    ...
    B -->|无标记| D[正常业务流程]
    C --> E[故障注入触发]
    E --> F[下游服务异常: 慢响应/500错误]
    F --> G[订单服务保护机制]
    G --> H[超时控制 connect/read]
    G --> I[熔断状态机: CLOSED/OPEN/HALF_OPEN]
    G --> J[重试与退避: 指数退避+限流]
    H --> K[验证是否按预期断开]
    I --> L[验证是否拦截并降级]
    J --> M[验证避免流量放大]
...
<img src="/interview-2026/images/diagram_java-architect_java-architect-148.svg" alt="核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
