...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:15<br/>存储选型要点"]:::deep
    N3["0:40<br/>合并逻辑要点"]:::deep
  end
  subgraph Outro["🎯 收尾"]
...
|----------|------------------|--------------|
| 目标追问 | 购物车为什么要用 Redis Hash 不直接用 MySQL？用户加购这么频繁吗？ | 加购/删除/改数量/勾选都是高频写（用户刷新页面就查），MySQL 行锁扛不住。Redis Hash 单 key 内多 field 操作原子且 O(1)。判断依据：QPS 监控——单用户日均操作 20+ 次，百万 DAU 就是 2000 万 QPS，MySQL 必挂 |
| 证据追问 | 你怎么证明跨端合并真的正确？用户反馈"App 加的商品 PC 看不到"怎么排查？ | 看 cart_merge_conflict_count（device 合并到 user 时的冲突次数）和 cart_inconsistency_report（用户反馈两端不一致的工单）。traceId 贯穿加购-合并-查询，能定位是合并漏了还是 Redis 写失败。监控 merge_success_rate（应 > 99.9%）|
| 边界追问 | 购物车用最终一致，但用户加购后立即刷新看不到（异步落 DB 还没完成）怎么办？ | 这不是最终一致问题——Redis 是主存，加购直接写 Redis（同步），用户刷新读 Redis 立即可见。异步落的只是 MySQL（持久化兜底）。Redis 挂了从 MySQL 恢复时有几秒窗口数据可能旧，但概率极低。监控 cart_write_to_read_latency（应 < 100ms）|
| 反例追问 | 给一个购物车合并出 bug 的真实反例？ | 同一 SKU 不同规格（红色 64G / 蓝色 128G）合并键没带 skuSpec，只按 skuId 合并，导致红色和蓝色被合并成一条（数量 3，规格丢失）。用户结算拿到错的规格。根因：合并键设计错。修复：合并键 = skuId + "_" + skuSpec。监控 sku_spec_loss_count |
| 风险追问 | 用户在 App 删了商品 A，PC 端同时还在改商品 A 数量，并发冲突怎么办？ | Redis Hash 单 field 操作原子（HDEL 和 HSET 互斥），不会出现"删了又被改回来"。但业务层可能展示不一致（PC 改数量成功但其实是新增了一条已删的商品）——所以 PC 操作前先校验 field 是否存在，不存在提示"商品已被其他端删除，请刷新"。监控 concurrent_cart_op_conflict |
| 验证追问 | 结算前强校验真的拦住了过期数据吗？ | 监控 checkout_validation_fail_rate（结算校验失败率，应 < 5%）。分类统计失败原因：sku_offline（下架）、price_changed（改价）、stock_insufficient（缺货）、limit_exceeded（超限购）。如果 price_changed 占比高，说明购物车展示价格太旧，要加查询时的实时价格刷新 |
| 沉淀追问 | 多业务线都有"用户暂存数据"场景（购物车/收藏夹/对比列表），你怎么避免重复造轮子？ | 沉淀 UserStash 通用框架——Redis Hash 存储 + 合并键策略 + 实时校验钩子。购物车/收藏夹/对比列表只声明合并键和校验规则。提供 stash_consistency_check 通用对账工具。监控按业务拆 stash_operation_qps 定位热点 |

### 现场对话示例

**面试官**：你说加购不锁库存，但秒杀场景用户加购了 100 件，结算时才发现库存只有 10 件，体验差怎么办？

**候选人**：购物车展示时实时查库存，库存不足直接标"仅剩 X 件"或"已售罄"（灰色不可勾选）。但这是"展示提示"不是"锁库存"——用户看到的库存是查询时刻的快照，结算时还会再校验一次。秒杀场景真正锁库存是在"下单"环节（创建订单扣预占库存），购物车不参与锁。监控 cart_to_checkout_stock_mismatch（加购时有货结算时没货的比例）。

**面试官**：那如果用户加购了秒杀商品但 30 分钟后才结算，库存早被抢光了，结算失败用户骂街怎么办？

**候选人**：秒杀商品特殊处理——加购时设短 TTL（如 5 分钟）"秒杀预留"，5 分钟内未下单自动从购物车移除。前端提示"秒杀价仅保留 5 分钟"。这变相是一种"软锁"——不真锁库存（不扣减），但用户感知上有时间窗。正常商品（非秒杀）不设这个 TTL，因为正常库存充足不需要紧迫感。监控 seckill_cart_timeout_rate。

**面试官**：未登录购物车合并到登录后购物车，取较大数量。但如果 device 购物车有 5 件（用户乱加的），user 购物车原本只有 2 件（认真选的），合并成 5 件用户可能不想要 5 件。
