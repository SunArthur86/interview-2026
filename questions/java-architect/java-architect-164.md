...
flowchart TD
    ...
    B -- 复杂/投诉 --> D[强模型 GPT-4]
    A --> E{第二道闸: 语义缓存}
    E -- 命中相似度>0.95 --> F[返回缓存答案]
    E -- 未命中 --> B
    A --> G{第三道闸: Token 预算/限流}
    G -- 超额/高频 --> H{第四道闸: 降级链}
    H -- 主模型挂 --> I[降级备用模型]
    I -- 备用挂 --> J[降级规则引擎]
    J -- 无匹配 --> K[返回兜底文案]
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:15<br/>模型路由要点"]:::deep
    N3["0:40<br/>语义缓存要点"]:::deep
  end
  subgraph Outro["🎯 收尾"]
...
|----------|------------------|--------------|
| 目标追问 | AI 应用成本治理，为什么不直接砍 token 单价（换便宜模型），要搞这么复杂？ | 换便宜模型降质量（弱模型答不准用户体验差）。成本治理是"按价值分配"——高价值场景保质量用强模型，低价值用弱模型+缓存。一刀切换便宜模型省了钱但流失了用户，ROI 为负 |
| 证据追问 | 你怎么证明模型路由真的省了钱且没伤体验？ | 监控 cost_per_query（应下降 50%+）、cache_hit_rate（应 30-50%）、misroute_rate（路由错误率，强模型误分弱模型降质量，应 < 10%）、user_satisfaction（NPS/点踩率，不应下降）。按维度（场景/用户/模型）拆成本看板 |
| 边界追问 | 语义缓存什么场景不能用？ | 时效敏感 query 不能缓存——"我的订单状态""当前库存""实时价格"。私有数据 query 要租户隔离缓存（A 用户的问题不能命中 B 用户的答案）。缓存 TTL 按 query 类型分级（FAQ 1 小时，订单 0 缓存） |
| 反例追问 | 给一个降级链反而坑了业务的真实反例？ | 大促时主模型超时降级弱模型，弱模型答退款争议时态度生硬（没同理心），用户从"想退款"升级到"投诉+差评"。修复：降级链要分场景——投诉/争议类 query 即使主模型慢也不能降级，宁可排队等强模型。监控降级后的 complaint_escalation_rate |
| 风险追问 | token 预算超额降级到弱模型，但如果用户是付费 VIP 也降级吗？ | VIP 不降级。预算按用户价值分配——VIP $10/天强制走强模型不降级，普通用户 $1/天超额降级。判断 user.isVip() 优先于预算检查。监控 vip_degrade_rate（应为 0，VIP 被降级是事故） |
| 验证追问 | 怎么验证限流真的拦住了恶意用户？ | 构造压测——脚本高频发超长 prompt，看 QPS 限流（5/s）和 token/min 限流（5000）是否触发。线上监控 top 10 用户的 token 消耗占比，如果 1% 用户占 30% 消耗说明限流没生效或阈值太松 |
| 沉淀追问 | 团队多 AI 业务都要成本治理，你怎么避免每业务各搞一套？ | 沉淀 AI 网关——统一接入模型路由 + 语义缓存 + 预算控制 + 降级链，业务只声明 query 类型 + 用户价值。提供 cost_dashboard 按业务/场景/模型拆解。共享评测集验证路由准确性。监控 each_business_cost_per_query |

### 现场对话示例

**面试官**：语义缓存相似度阈值 0.95，怎么定的？定低了（0.85）会怎样？

**候选人**：0.85 会导致"假命中"——"iPhone 15 价格"和"iPhone 15 评测"embedding 相似度可能 0.88，命中同一缓存但答案完全不对。0.95 是经验值（实测 query pair 在 0.95 以上语义基本等价）。定阈值要跑评测集——人工标注 1000 个 query pair（相同/不同），算不同阈值下的 precision/recall，选 precision > 0.99 的点。

**面试官**：那如果用户问"退款"两次，第一次答完了，第二次缓存命中直接返回——但中间用户的订单状态变了（已退款），返回旧答案怎么办？

**候选人**：这就是时效性问题。"退款进度"类 query 不进语义缓存（或 TTL 5 分钟）。缓存写入前判断 query 类型——涉及用户实时状态的（订单/库存/物流）打 no_cache 标签，只缓存通用 FAQ（退货政策/运费规则）。监控 cache_stale_rate（用户点踩缓存的次数，反映过期命中率）。

**面试官**：成本看板给 CFO 看，他问"这个 AI 客服到底值不值"，你怎么算 ROI？
