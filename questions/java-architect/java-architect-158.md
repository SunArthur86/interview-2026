...
- 评估
feynman:
  essence: 向量库选型的本质是"在数据规模、延迟、成本、运维能力四个约束下做取舍"。Milvus（分布式、亿级、HNSW/IVF）适合大规模自建；pgvector（PG
    扩展、ACID、< 千万级）适合中小规模且已有 PG；Elasticsearch（8.x dense_vector）适合已有 ES 栈且要混合检索。评估的核心是"用标注评测集跑召回率/延迟/成本三指标横向对比"，不靠"听说好用"。
  analogy: 像选仓库——Milvus 是大型自动化立体仓（容量大但要专业运维），pgvector 是便利店后仓（小但和业务库在一起管理方便），ES 是已有物流中心加了个冷藏区（复用现有栈）。
  first_principle: 向量检索的瓶颈是"高维空间最近邻搜索（ANN）"，暴力精确搜索 O(N) 不可行，必须用近似算法（HNSW/IVF/PQ）把复杂度降到
    O(logN)。不同算法在召回率、内存占用、构建速度上各有取舍，这就是选型的技术根基。
  key_points:
...
first_principle:
  ...
  rebuild: 用 ANN 算法建索引——HNSW 把向量组织成多层导航图（查询时从顶层逐层下钻，O(logN)），IVF 用 K-means 聚类后只搜索最近的
    nprobe 个簇。大规模用 Milvus（分布式 + 多索引可选），中小规模复用 PG（pgvector 的 HNSW 索引）。评测用标注集跑 recall@10
    和 p99。
follow_up:
- HNSW 的 M 和 efConstruction 怎么调？——M（图的度数）越大召回越高但内存越多，常用 16-48；efConstruction（构建时搜索宽度）越大索引质量越好但构建慢，常用
  200-500。ef（查询时搜索宽度）动态调，精度优先调大、速度优先调小。
- Milvus 和 pgvector 性能差多少？——千万级以内 pgvector HNSW 和 Milvus 差距 < 20%（延迟都在 10ms 内），但亿级
  Milvus 分布式优势明显（分片并行）。pgvector 单机内存是天花板。
- 向量维度怎么选？——1536（OpenAI/bge-large）精度好但贵，768（bge-base）性价比高，384（bge-small）快但召回略低。按评测集
  recall@k 差距是否值得成本翻倍来定。
- 索引怎么更新不重建？——HNSW 支持在线插入但删除是软删除（标记 tombstone，定期 compact）。大量更新场景用"双 collection 切换"——新
  collection 后台重建，建好原子切换。
- 怎么评估选型对不对？——建 100-1000 条标注评测集（query + 相关文档），跑 recall@10、MRR、qps、p99_latency。横向对比候选库，差
  5% 以内优先选运维简单的。
memory_points:
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:15<br/>三大索引算法要点"]:::deep
    N3["0:40<br/>选型四要素要点"]:::deep
  end
  subgraph Outro["🎯 收尾"]
