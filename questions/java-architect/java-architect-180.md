...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:15<br/>像快递调度调度中心每分钟问每个快"]:::concept
    N3["0:40<br/>心跳探活要点"]:::deep
    N4["1:05<br/>任务重分配要点"]:::deep
    N5["1:30<br/>幂等执行要点"]:::deep
  end
  subgraph Outro["🎯 收尾"]
...
|----------|------------------|--------------|
| 目标追问 | 调度层为什么要保证 exactly-once？at-least-once + 业务幂等不行吗？ | 调度层做不到真 exactly-once（网络不可靠——任务执行成功响应丢了，调度中心以为失败重试）。所以业界共识：调度层 at-least-once（任务至少执行一次，可能重复）+ 业务层幂等（重复执行不产生副作用）= 恰好一次的"效果"。真 exactly-once 要两阶段提交，成本高得离谱（锁、协调者），不值得 |
| 证据追问 | 你怎么证明 failover 真的把失效节点的任务接住了、没丢任务？ | 监控 failover_count（失效转移次数，应 > 0 且每次都对应真实节点故障）、failover_task_loss_count（转移期间丢的任务数，应 = 0）、retry_success_rate（重试成功率，应 > 95%，低于说明重试策略有问题）、idempotency_violation_count（幂等违规数——重复扣款/重复发货，应 = 0）、task_claim_conflict_rate（多节点抢占冲突率，应 < 1%）|
| 边界追问 | 30 秒心跳间隔、3 次失败（90 秒）标记失效——这阈值怎么定？ | 90 秒是 trade-off：太短误判（网络抖动 30 秒、Full GC 停顿 20 秒都会误杀健康节点），太长恢复慢（任务延迟到 5 分钟才发现节点挂）。90 秒能容忍大多数瞬时抖动（GC/网络），又能及时恢复。心跳间隔还要考虑执行器数量——N 个执行器 × 30 秒一次 ping，调度中心 QPS 要够。监控 false_dead_mark_rate（误判死亡率，应 < 0.1%）|
| 反例追问 | 给一个心跳误判导致任务重复执行的反例？ | 节点 A 在 Full GC 停顿 100 秒（超过 90 秒心跳阈值），调度中心以为 A 死了，把 A 正在执行的"订单超时关闭"任务重新分配给节点 B。B 执行成功关闭订单。GC 结束后 A 恢复，继续执行原任务——但订单已被 B 关闭，A 重复执行（重复回滚库存）。根因：心跳阈值 < GC 停顿。修复：幂等兜底（状态机 CAS：UPDATE WHERE status='PENDING'，B 改成 CLOSED 后 A 的 CAS 失败跳过）。监控 false_dead_mark_rate 和 idempotency_skip_count（幂等跳过次数）|
| 风险追问 | 调度中心本身挂了怎么办？任务不就全停了？ | 单调度中心是单点。对策：调度中心多副本 + ZK/etcd 选主（LeaderSelector）。只有 leader 调度，follower 待命。leader 挂了 ZK 自动选新 leader（秒级切换）。任务状态存在 DB（不在调度中心内存），新 leader 接管后从 DB 恢复任务状态。风险：脑裂（ZK 网络分区导致双 leader）。用 ZK 的 ephemeral node + watch 保证唯一 leader。监控 leader_switch_count（leader 切换次数，应 < 每天 1 次）|
| 验证追问 | 幂等真的拦住了所有重复执行吗？你怎么证明？ | 三种幂等方案各有验证：唯一约束——监控 DuplicateKeyException 抛出次数（每次都是一次重复被拦截，应 > 0 说明真有重复尝试被拦）；状态机 CAS——监控 CAS 失败次数（updated=0，说明状态已被改走重复跳过）；分布式锁——监控 lock_contention_skip_count（拿不到锁跳过的次数）。线上混沌测试——故意制造重复（重发 MQ 消息），验证幂等拦截。监控 idempotency_violation_count（应 = 0，非 0 说明幂等失效）|
| 沉淀追问 | 多业务都要定时任务（订单关闭/积分结算/库存对账），怎么避免每业务自建调度？ | 沉淀通用 SchedulerSDK——任务注册（CRON 表达式 + 执行器路由策略）+ 心跳 + failover + 幂等通用，业务只声明任务逻辑。提供 failover_count 和 retry_success_rate 看板按业务拆。共享幂等组件（注解 @Idempotent(key="orderId")）。监控 task_execution_duration 按业务对比，长任务告警 |

### 现场对话示例

**面试官**：你说调度层 at-least-once + 业务幂等 = 恰好一次效果。但如果业务幂等有 bug（比如唯一约束漏建），重复执行真发生了，怎么办？

**候选人**：分两步。第一是发现——监控 idempotency_violation_count（幂等违规数，通过对账发现重复扣款/重复发货）。这个指标非 0 立即告警，意味着幂等失效。第二是修复——立即关停重复触发的任务（关 flag），对已重复执行的业务做对账修复（如重复扣款的退款）。根因修复幂等 bug（补唯一约束/CAS 条件）。预防：所有有副作用的任务 Code Review 必查幂等键，CI 强制扫描 @Idempotent 注解覆盖率。

**面试官**：任务版本号（乐观锁）防并发抢占，但如果两个节点同时 CAS 同一任务，失败的那个怎么办？

**候选人**：CAS 失败的节点放弃这个任务（已被别人抢走），去抢下一个 PENDING 任务（SELECT WHERE status='PENDING' LIMIT 1）。这是乐观锁的代价——失败要重试抢别的任务。但实际中冲突率很低（任务量大、节点少，两个节点同时看中同一任务的概率 < 1%）。监控 task_claim_conflict_rate（抢占冲突率），如果 > 5% 说明任务分配不均（可能某节点空闲其他忙），优化任务分配策略（按负载均衡分配而不是自由抢占）。
