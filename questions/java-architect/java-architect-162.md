...
|----------|------------------|--------------|
| 目标追问 | Agent 多步任务为什么不能直接 try-catch 重试，非要搞状态机和 Saga？ | 因为有副作用的步骤重试会重复执行（重复扣库存、重复发券）。无状态接口可以 try-catch，有副作用必须先补偿已执行的步骤再重试。判断依据：看 saga_log 里有没有 EXECUTED 状态的副作用步骤 |
| 证据追问 | 你怎么证明补偿链路真的可靠？ | 看三个指标：compensation_rate（触发补偿比例，应 < 5%，过高说明工具不稳）、compensation_success_rate（补偿成功率，应 > 99%）、manual_intervention_rate（需人工的比例，应 < 1%）。traceId 回放任意 taskId 看完整轨迹 |
| 边界追问 | Saga 补偿能覆盖所有失败场景吗？什么场景必须人工介入？ | 不可逆操作不能自动补偿——已到账的退款（银行已结算）、已发送的短信、已签收的实物。这些标记 `requires_manual=true`，状态机转 PAUSED_ERROR 告警人工。补偿只覆盖可逆操作 |
| 反例追问 | 给一个补偿链路本身出问题的真实反例？ | LLM 决策"补偿后重试"但每次重试都到第 4 步失败，导致无限循环补偿-重试-补偿。修复：加全局 retry_budget（同一 taskId 最多重试 3 次，超过强制 PAUSED_ERROR）。监控 infinite_loop_alert |
| 风险追问 | 状态机 + checkpoint 持久化太频繁会不会影响性能？ | 每步 checkpoint 写 DB 是同步阻塞。高频任务（10 步/秒）会卡。优化：异步 checkpoint（执行不阻塞，写 DB 异步队列）或每 N 步一次（牺牲恢复粒度）。但关键里程碑（有副作用的步骤）必须同步 checkpoint |
| 验证追问 | 怎么验证幂等真的有效？ | 混沌测试：人为在 step 3 后杀进程，重启后看是否从 step 3 恢复而非从头；人为让同一 taskId 重试 5 次，看 saga_log 是否只有一份副作用记录。监控 idempotency_violation_count（应为 0） |
| 沉淀追问 | 多个 Agent 业务都要多步任务，你怎么避免每业务重写状态机？ | 沉淀通用 AgentTask 框架——业务只注册 tool + compensateAction，状态机/checkpoint/Saga 执行器通用。提供 tool_registry 标注哪些工具有副作用需注册补偿。监控按业务线拆 compensation_rate 定位不稳工具 |

### 现场对话示例

**面试官**：你说每步执行前注册补偿动作，但如果注册后还没执行进程就崩了，saga_log 里没有这条记录怎么办？

**候选人**：注册和执行是两步。我的设计是：先写 saga_log（status=PENDING），再执行工具，执行成功更新 status=EXECUTED。进程崩在 PENDING 状态——恢复时发现 PENDING 的步骤没执行成功，不需要补偿（副作用没产生），直接跳过继续。所以 checkpoint 要包含 saga_log 全量状态。

**面试官**：那如果执行成功但更新 status=EXECUTED 之前崩了呢？工具实际执行了但 saga_log 还标 PENDING。

**候选人**：这就是"执行了但没记录"的危险场景。所以工具本身必须幂等——恢复时不知道有没有执行，重试一次幂等保证无副作用。比如 refund 用 idempotency_key（taskId+stepId），重复调渠道返回上次结果。前提是所有工具都接入 IdempotentToolExecutor，不允许裸调。

**面试官**：如果 LLM 在补偿过程中又生成了新的正向步骤呢？比如补偿退款失败，LLM 决定改发优惠券补偿。

**候选人**：状态机约束——COMPENSATING 状态下只允许补偿动作，不允许新正向步骤。LLM 想发券必须等补偿完成（转 FAILED 或 COMPENSATED）再开新任务。这是为了防补偿和正向步骤交错产生新的不一致。新任务是新 taskId 独立 saga_log，不混入旧任务的补偿链。

## 核心知识点图

<img src="/interview-2026/images/diagram_java-architect_java-architect-162.svg" alt="核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
