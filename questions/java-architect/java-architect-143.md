...
flowchart TD
    ...
    subgraph Redis集群 ["Redis集群: 100个分桶并行"]
        D
    end
    D -- "扣减成功" --> E["生产MQ消息"]
    D -- "桶内无库存" --> F["Lua脚本<br/>跨桶库存迁移"]
    F -- "迁移成功" --> E
    F -- "全部售罄" --> G["返回失败/售罄"]
    E --> H["MQ消费者异步落库"]
    subgraph MySQL存储 ["MySQL存储"]
        H --> I["deduct_id幂等校验"]
        I --> J["库存表最终一致性记录"]
    end
    D -. "Redis故障" .-> K["降级: DB行锁限流"]
    K --> J
```
...
<img src="/interview-2026/images/diagram_java-architect_java-architect-143.svg" alt="核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
