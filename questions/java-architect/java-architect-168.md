...
feynman:
  ...
  analogy: 僐购平台收到你一个订单——有书（北京仓）、有生鲜（冷链仓，上海）、有手机（第三方卖家自己发货）。系统要拆成 3 个包裹，分别从 3 个地方发，你收到
    3 个快递。拆错了（生鲜和书发一仓）要么生鲜坏了要么发不出。
...
- 路由算法怎么选最优仓？——多目标优化。输入：收货地址、商品列表、各仓库存。候选：有库存且覆盖收货地的仓库。打分：距离分 + 时效分 + 成本分 + 库存健康分。选
  top1。
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:15<br/>僐购平台收到你一个订单有书"]:::concept
    N3["0:40<br/>三维拆单要点"]:::deep
    N4["1:05<br/>路由策略要点"]:::deep
  end
  subgraph Outro["🎯 收尾"]
