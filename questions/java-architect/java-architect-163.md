...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:15<br/>转换层模式要点"]:::deep
    N3["0:40<br/>结构化输出要点"]:::deep
  end
  subgraph Outro["🎯 收尾"]
...
|----------|------------------|--------------|
| 目标追问 | 为什么不让 LLM 直接操作数据库？加个 SQL 工具不更灵活？ | LLM 是概率模型，可能幻觉出 `DELETE FROM orders` 或被 prompt injection 诱导。即使加权限，LLM 的 WHERE 条件可能错（"删除用户 888 的订单"理解成"删除 888 笔订单"）。转换层硬校验不可省 |
| 证据追问 | 你怎么证明转换层真的拦住了 LLM 的错误决策？ | 监控三道闸的拦截率：schema_fail_rate（格式错，应 < 2%）、rule_fail_rate（业务规则不通过，反映 LLM 推荐质量）、perm_fail_rate（权限拒绝）。再监控 llm_triggered_error_rate（LLM 决策导致的业务错误，应 < 传统人工错误率）和 revert_rate（事后撤销，应 < 1%） |
| 边界追问 | 三道校验够吗？什么场景必须加 HITL（人工确认）？ | 高敏操作必须 HITL：大额退款（> 1000 元）、批量操作（一次退多单）、不可逆操作（删除数据、转账）。判断依据：操作的"资金风险 × 不可逆性"矩阵，高风险强制人工 confirm |
| 反例追问 | 给一个 LLM 进业务流程出事的真实反例？ | LLM 理解"帮我退订单 888 的货"时把 888 解析成订单号，但用户实际指的是订单 8888（口误）。schema 校验通过（格式对）、规则校验通过（订单存在且属于用户）。只有"二次确认"环节（"您要退订单 888 共 320 元，确认？"）用户发现错了。所以高敏操作必须有二次确认，不能 LLM 一说就执行 |
| 风险追问 | function calling 的 schema 校验失败重试 2 次仍失败，降级人工——但用户在等，体验差怎么办？ | 降级不是无响应，是返回"我没能理解您的需求，能换种说法吗 / 请点击这里人工客服"。同时记录失败 case 进评测集优化 prompt。长期看 schema_fail_rate 要 < 2%，否则是 prompt 或模型选型问题 |
| 验证追问 | 怎么验证 prompt injection 真的被拦住了？ | 红队对抗测试——用攻击 LLM 批量生成注入变体（"忽略指令"的 100 种说法 + 越权请求），统计 attack_bypass_rate（突破率，应 < 0.1%）。线上监控 perm_fail_rate 突增（可能是新型攻击） |
| 沉淀追问 | 团队多个 LLM 业务都要进业务流程，你怎么避免每业务重写校验？ | 沉淀 LlmToBusinessGateway 通用框架——业务只注册 schema + BusinessRuleValidator + 权限策略，转换层/幂等/审计通用。提供 LLM action_log 标准表结构支持统一回溯撤销。监控按业务维度拆 llm_triggered_error_rate 定位弱业务 |

### 现场对话示例

**面试官**：你说 schema 校验失败重试 2 次，但重试时 LLM 可能还是输出同样的错误（temperature=0 时确定性问题）。怎么办？

**候选人**：重试不是简单重发，是带反馈重试——把上次的错误信息拼进 prompt（"你上次的输出 orderId 字段缺失，请重新输出完整 JSON"）。temperature=0 时加反馈能改变输出。如果带反馈重试 2 次仍失败，说明模型能力不足或 prompt 设计有问题，降级人工并进评测集。

**面试官**：那如果 LLM 一直输出格式对但语义错的决策呢？比如 orderId 格式对但每次都填错人的订单。

**候选人**：这就是规则校验的作用——查库验证 orderId 真实存在 + 属于当前 session 用户。LLM 填的 userId 不信任，强制用 session 的 userId 覆盖。监控 rule_fail_rate，如果某类规则频繁失败（如"订单不属于用户"占 80%），说明 LLM 在越权推荐，要查 prompt 是不是引导错了。

**面试官**：审计日志里记了 LLM 原始输出，但 LLM 输出可能含敏感信息（用户隐私），怎么处理？
