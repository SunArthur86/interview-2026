...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:15<br/>像景区年卡管理发卡、刷卡进园类比"]:::concept
    N3["0:40<br/>生命周期要点"]:::deep
    N4["1:05<br/>发放要点"]:::deep
  end
  subgraph Outro["🎯 收尾"]
...
|----------|------------------|--------------|
| 目标追问 | 权益发放为什么要走 MQ 异步，直接同步 DB insert 不行吗？ | 双 11 亿级发放同步 DB 扛不住（行锁+连接池打满）。MQ 削峰——发券请求先进 Kafka 排队，消费端批量拉批量写（batch insert 500 条），吞吐 10 万/秒。同步发放会让 DB 在大促首秒挂掉 |
| 证据追问 | 你怎么证明券没多发也没漏发？ | 监控 issue_overcount（实际发放数 > 应发数的批次）、issue_undercount（漏发）、redeem_concurrent_conflict_rate（核销并发冲突率，应 < 0.1%）。每批次有 batch_id 幂等键，可重放核对。DB 唯一约束（uk_benefit）物理防重 |
| 边界追问 | 乐观锁 CAS 和 DB 唯一约束双保险，但极端情况两个都失效怎么办？ | 极端场景：乐观锁的 version 字段被人为改错（脏数据），DB 唯一约束被误删。对策：核销后走 T+1 对账——同一 benefit_no 在 t_benefit_usage 出现多条记录即异常告警。监控 redeem_duplication_count（应为 0，出现立即人工核查）|
| 反例追问 | 给一个核销重复的真实反例？ | 用户网络抖动重复提交核销请求，乐观锁第一个成功（version 5→6），第二个用旧 version=5 失败正常。但如果应用层缓存了 benefit 对象（version=5），第二个请求用缓存的 version 也能 CAS 成功（因为 DB 还是 5）。根因：缓存脏读。修复：CAS 前强制从 DB 读最新 version，不信任缓存。监控 concurrent_redeem_leak |
| 风险追问 | 订单取消回退券（USED→ACTIVE），但如果券已过期了不退，用户投诉"我的券为什么没了"怎么办？ | 业务规则要透明——领券时明确"订单取消退券，但券过期不退回"。前端在订单详情标注"此订单使用的券有效期至 X，取消后券不退回"。技术上：回退前检查 expire_at，过期则记录"过期未退"日志但不回退状态。监控 expired_voucher_complaint（投诉率，高则业务规则要调）|
| 验证追问 | 双 11 亿级发放，怎么验证真的发到了所有人？ | 分桶校验——按 userId hash 分 100 桶，每桶抽样核对"应发 N 人 vs 实际发放 M 人"。M < N 说明漏发（重发该批次）。监控 issue_completion_rate（应 = 100%）。发放完成后提供"未发放清单"给运营复核 |
| 沉淀追问 | 多业务（优惠券/积分/会员权益/赠品）都有生命周期管理，你怎么避免重复造轮子？ | 沉淀通用 BenefitLifecycleService——状态机（CREATED→ACTIVE→USED/EXPIRED）+ 发放/核销/回退/过期通用，业务只声明权益配置和发放人群。提供 benefit_trace 看板（benefitNo 全链路追溯）。监控按业务拆 issue_success_rate 和 redeem_success_rate |

### 现场对话示例

**面试官**：你说券码从预生成池取，但池子怎么生成？亿级发放池子要存哪？

**候选人**：预生成池离线生成——用雪花算法+加密签名生成 N 亿个唯一券码，存 Redis Set 或独立表。发放时从池 LPOP 取一个绑定 userId。池子补充策略：剩余 < 20% 时后台异步生成补充。监控 pool_size（剩余券码数，低水位告警）。池化的好处：发放时不用实时生成（避免雪花算法在秒级峰值下的时钟回拨问题）。

**面试官**：核销时乐观锁和唯一约束都过了，但实际业务副作用（如满减抵扣）执行失败了，券状态已是 USED 怎么办？

**候选人**：核销要和业务副作用在同一事务。`@Transactional` 包住：CAS 更新券状态 USED + INSERT 核销记录 + 执行业务（订单减优惠金额）。任一步失败整体回滚（券状态不变）。如果事务提交后业务系统（如订单）回调失败，走补偿——监听订单失败事件，回退券状态 ACTIVE。监控 redeem_commit_but_business_fail（提交成功但业务失败的比例，应 < 0.1%）。
