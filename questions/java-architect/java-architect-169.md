...
- 对账
feynman:
  essence: 售后退款链路的本质是"订单状态机的逆向流程 + 资金回流一致性"。逆向流程：申请售后 → 审核 → 退货入库 → 退款，每步有状态机约束防止非法跳转。资金回流是核心难点：原路退回（微信/支付宝）要和支付渠道对接，退款可能失败（渠道超时/账户注销），必须幂等重试
    + 对账兜底。
  analogy: 像超市退货——你拿商品和小票到服务台（申请售后），服务员核对（审核），商品退回货架（入库），钱原路退回你的卡（退款）。如果退钱时刷卡机坏了（渠道超时），要登记后续补退（对账兜底）。
  first_principle: 退款是"已完成的支付的反向操作"，但比支付更难——支付是"钱从用户到商家"，退款是"钱从商家退回用户"，需要原路返回（微信退微信、支付宝退支付宝）。退款可能因渠道问题失败，必须幂等重试
    + T+1 对账发现差异。
  key_points:
...
first_principle:
  ...
  rebuild: 售后单走逆向状态机（APPLIED → APPROVED → RETURN_RECEIVED → REFUNDING → REFUNDED），每步状态机校验。退款走原路退回
    + 幂等键（退款单号）防重复。退款失败重试（指数退避），仍失败进对账队列。T+1 对账：平台退款记录 vs 渠道流水，差异自动补偿。
follow_up:
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:15<br/>逆向状态机要点"]:::deep
    N3["0:40<br/>退款渠道要点"]:::deep
  end
  subgraph Outro["🎯 收尾"]
...
|----------|------------------|--------------|
| 目标追问 | 退款为什么必须原路退回？退到平台余额不是更简单？ | 支付渠道约束 + 用户信任。微信支付的钱在微信体系，退到余额相当于"扣了用户现金给代币"，用户感知差且合规风险。原路退回是支付协议要求。判断依据：看渠道协议——大多数支付渠道要求退款必须原路，否则商户违规 |
| 证据追问 | 你怎么证明退款链路没漏退也没多退？ | 监控 reconcile_diff_rate（T+1 对账差异率，应趋近 0）、refund_idempotency_violation（幂等失效导致重复退款，应 = 0）、false_success_refund（平台标记成功但渠道无记录的"虚假退款"，应 = 0）。每笔退款有 refundNo 可追溯 |
| 边界追问 | 部分退款按比例分摊优惠，但用户用了"满减券"且券已过期，退哪件商品扣多少优惠怎么算？ | 按实付比例分摊。订单原价 100 满减 20 实付 80，退原价 30 的商品退 30×(80/100)=24。券过期不影响（券的优惠在支付时已生效，退款按当时实付算）。但如果全单退，过期券按规则可能不退回（业务策略）|
| 反例追问 | 给一个退款出事的真实反例？ | 用户重复点击"申请退款"，并发请求导致幂等键失效（refundNo 生成不唯一），渠道侧退了两次。根因：refundNo 基于 timestamp 生成并发下重复。修复：refundNo = afterSaleId（强唯一），DB 唯一约束兜底。监控 duplicate_refund_count（应为 0）|
| 风险追问 | 退款超时（渠道未响应）不能假设成功也不能假设失败，但如果一直超时用户等不及怎么办？ | 超时进查单队列，调渠道查询接口确认。查单有结果按实际状态更新；查单也超时（极罕见）进人工。用户侧展示"退款处理中，预计 X 小时到账"（基于历史平均时长），不让用户无限等。监控 refund_unknown_stuck_count（长期未知的退款数）|
| 验证追问 | T+1 对账真的发现了差异吗？还是形同虚设？ | 看对账报表——每天 reconcile_diff_count（差异笔数）和分类（false_success / amount_diff / unknown_refund）。如果有差异但没人处理，对账就是摆设。要对差异设 SLA——false_success 1 小时内补退，amount_diff 24 小时内核实。监控 reconcile_to_resolution_time |
| 沉淀追问 | 多业务（退货退款/仅退款/补偿）都要走资金回流，你怎么避免每业务重写？ | 沉淀通用 RefundGateway——原路退回 + 幂等键 + 重试 + 查单 + 对账通用，业务只声明退款金额计算逻辑（RefundCalculator）。提供 refund_trace 通用看板（refundNo 全链路追溯）。监控按业务拆 refund_success_rate 和 avg_refund_time |

### 现场对话示例

**面试官**：退款按比例分摊优惠，但如果订单有多个优惠（满减+优惠券+会员折扣），分摊顺序怎么算？

**候选人**：统一按"实付/原价"比例分摊，不分优惠类型。订单原价 100，各种优惠合计 20，实付 80。退原价 30 的商品退 30×(80/100)=24。不区分哪个优惠贡献了多少——因为对用户来说只关心"我付了 80 退多少"。这样避免复杂的优惠分摊（满减按门槛分？券按面额分？会员折扣按比例分？）。简单且对用户透明。

**面试官**：那如果退的商品正好是触发满减的那件呢？比如满 200 减 30，退了 50 元的商品后订单不满 200 了，满减还成立吗？

**候选人**：这是业务策略问题。两种处理：(1) 不追溯——满减在支付时已生效，部分退货不重新计算（用户友好，平台让利）；(2) 追溯——退了关键商品导致不满门槛，退款时扣除已享受的满减（平台严谨，用户不爽）。JD 早期用(1)（不追溯提升体验），现在部分场景用(2)防薅羊毛。这个要业务方决策，技术层提供两种计算器可选。
