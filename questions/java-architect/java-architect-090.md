...
flowchart TB
    ...
    subgraph Gateway["模型服务网关（Model Gateway）"]
        Access["接入抽象<br/>多供应商 统一 API"]
        Flow["流量治理<br/>限流熔断 降级重试"]
        Cost["成本控制<br/>路由缓存 预算审计"]
        Obs["可观测<br/>延迟成本质量 traceId 串联"]
    end
    Gateway --> OpenAI["OpenAI (主)"]
    Gateway --> Qwen["通义/文心 (备)"]
    Gateway --> Self["自部署 Llama/Qwen"]
```
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:15<br/>像一个多语种翻译调度台有贵的金牌"]:::concept
    N3["0:40<br/>多供应商统一抽象要点"]:::deep
    N4["1:05<br/>降级链要点"]:::deep
    N5["1:30<br/>模型路由要点"]:::deep
  end
  subgraph Outro["🎯 收尾"]
