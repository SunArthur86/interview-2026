...
```mermaid
flowchart TD
    OOM[OutOfMemoryError] --> LOC{定位区域}
    LOC -->|Java heap space| HEAP[堆溢出]
    LOC -->|Metaspace| META[元空间溢出]
    LOC -->|Direct buffer| DIR[直接内存溢出]
    LOC -->|StackOverflow| STK[栈溢出]
    HEAP --> MAT[MAT 分析泄漏/大对象]
    META --> DC[动态生成类过多]
    DIR --> NIO[查 NIO ByteBuffer]
    STK --> REC[查递归深度]
