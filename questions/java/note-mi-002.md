...
flowchart TD
    subgraph S1[任务提交与分配]
        ...
        B -- 否 --> D{有界队列<br/>是否已满}
        D -- 否 --> E[任务进入<br/>有界队列等待]
        D -- 是 --> F{活跃线程数<br/>是否达到最大线程数}
        F -- 否 --> G[创建非核心线程<br/>执行任务]
        F -- 是 --> H[触发拒绝策略]
    end
    subgraph S2[线上拒绝策略选择]
        H --> I{任务核心级别}
        I -- 核心链路 --> J[AbortPolicy<br/>快速失败防积压]
        I -- 可降级异步 --> K[CallerRunsPolicy<br/>主线程背压限流]
        I -- 跨进程且绝不丢 --> L[自定义策略<br/>兜底写Kafka重试]
    end
    subgraph S3[线上监控与动态调整]
        M[暴露Metrics指标] --> N[监控: 队列积压与拒绝率]
        N --> O{是否超过阈值}
        O -- 是 --> P[动态调整<br/>核心/最大线程数]
    end
    C --> M
    G --> M
    E --> M
