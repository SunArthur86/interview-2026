...
graph TD
    ...
    class A2 process
    class A3 decision
    class ArrayDeque special
    class ArrayList error
    class Arrays info
    class B1 start
    class B2 process
    class B3 decision
    ...
    class next decision
    class prev special
    subgraph S1[ArrayList 连续内存数组]
        A1[扩容机制<br/>Arrays.copyOf] --> A2[CPU缓存命中率极高<br/>连续内存空间局部性]
        A2 --> A3[综合场景首选<br/>覆盖95%工程需求]
    end
    subgraph S2[LinkedList 双向链表]
        B1[Node对象分散<br/>prev/next/item] --> B2[CPU缓存命中极低<br/>Cache Miss严重拖慢速度]
        B2 --> B3[实现Deque接口<br/>可作为队列或栈]
    end
    S1 -- 随机访问O1<br/>插入删除ON --> S2
    S2 -- 随机访问ON<br/>已知节点插入O1 --> S1

    subgraph S3[进阶替代方案]
        C1[高频出队入队场景]
        C2[ArrayDeque<br/>循环数组无对象分配]
        C3[性能高于LinkedList<br/>2至5倍]
        C1 --> C2 --> C3
    end
```
