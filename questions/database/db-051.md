...
flowchart TB
    ...
    class MyISAM special
    class Pluggable error
    class Storage info
    class analyzer start
    class cache process
    ...
    class engine info
    class exec start
    class innodb process
    class myisam decision
    class opt special
    client["客户端"]
    conn["连接器"]
    cache["查询缓存（8.0 已移除）"]
...
flowchart LR
    ...
    subgraph Core["讲解"]
        B["查询流程口诀：连接、缓存、分析、优化、执行（注意8.…"]:::core
        C["两阶段提交：一备二写三提交，保证Redo和Binlo…"]:::deep
    end
