...
flowchart TD
    ...
    M1 --> A1[AtomicReference 原子替换旧表引用]
    M2 --> A2[AtomicReference 原子替换旧表引用]

    subgraph Concurrent Read [并发无锁读抽奖]
        A1 --> R1[用户线程1读本地新表抽奖]
        A1 --> R2[用户线程2读本地新表抽奖]
        A2 --> R3[用户线程3读本地新表抽奖]
    end
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["【滴滴面经】如果奖品概率配置临时变了，概率查找表怎么更新？"]:::intro
    end

    subgraph Core["讲解"]
        B["核心矛盾：读写频率极度不对称（读多写少），直接修改数…"]:::core
        C["解决方案：写时复制（COW），先构建完整新表，再通过…"]:::deep
    end
