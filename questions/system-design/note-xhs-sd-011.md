...
first_principle:
  ...
  derivation: Redis单节点QPS上限约10万。明星官宣场景下，单个热点Key的QPS可能达到百万级。即使Redis集群能扛住，网络带宽和DB回源也会成为瓶颈。因此需要：1)
    让请求尽量在客户端/本地缓存命中（减少网络IO）；2) 把单个热点Key分散到多个Key和多个节点（减少单点压力）；3) 极端情况主动降级（保住系统不死比返回正确数据更重要）
