...
- 事务隔离
feynman:
  essence: InnoDB 用 undo log 版本链 + read view 实现 MVCC，让读写不互相阻塞；RC 每次 select 建 read
    view（看到最新已提交），RR 首次 select 建 read view 后复用（可重复读）。
...
flowchart TD
    ...
    class before start
    class gap process
    class innodb_lock_waits decision
    class locks special
    class qty error
    class rec info
    class roll_pointer start
    class stock_slot_0 process
    subgraph 事务并发控制
        A[RC 读已提交]
        B[RR 可重复读]
    end
    ...
    subgraph 库存扣减方案
        F[Redis Lua 前置扣减热点]
        G[MySQL 乐观锁 UPDATE WHERE qty>=1]
        H[热点分桶 stock_slot_0-9]
    end

    subgraph 监控与排查
        I[死锁日志 locks gap before rec]
        J[innodb_lock_waits]
    end
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:12<br/>4 级隔离"]:::concept
    N3["1:04<br/>能力/参数"]:::deep
    N4["1:56<br/>RC 每次读建 read vie"]:::deep
  end
  subgraph Outro["🎯 收尾"]
