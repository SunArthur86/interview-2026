...
- ThreadPoolExecutor
feynman:
  essence: 线程池用"核心线程+队列+非核心线程+拒绝策略"四级兜底应对流量洪峰；阿里规约禁止 Executors，必须 new ThreadPoolExecutor
    且队列有界。
...
flowchart TD
    ...
    class active start
    class br process
    class corePoolSize decision
    class maxPoolSize special
    class queue error
    class size info
    A["商品查询请求"] --> B{"当前活跃线程<br/>< corePoolSize?"}
    B -- "是" --> C["创建核心线程执行"]
    B -- "否" --> D{"阻塞队列已满?<br/>queue.size < 2000"}
    D -- "否" --> E["入队等待执行"]
    D -- "是" --> F{"达到最大线程数?<br/>active < maxPoolSize"}
    F -- "否" --> G["创建非核心线程执行"]
    F -- "是" --> H{"触发拒绝策略"}
    H --> I["CallerRunsPolicy<br/>Tomcat线程执行反压"]
    H --> J["AbortPolicy<br/>抛异常并降级返回缓存"]
    ...
    subgraph 根因与治理
        M["DB慢查询导致<br/>socketRead阻塞"]
        N["配置socketTimeout<br/>快速失败"]
        O["Sentinel熔断降级"]
        M -- "根治" --> N
        M -- "隔离" --> O
    end
