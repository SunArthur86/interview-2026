...
flowchart TD
    ...
    C --> D{"另一线程进入?"}
    D -- "是 (撤销偏向)" --> E["轻量级锁<br/>CAS自旋"]
    B -- "低竞争 (交替执行)" --> E
    E --> F{"CAS自旋成功?"}
    F -- "失败达到阈值" --> G["重量级锁<br/>OS Mutex Park"]
    G --> H["内核态阻塞与唤醒"]
    F -- "成功" --> I["执行同步代码"]
    G --> I
    I --> J["解锁并触发JMM可见性"]
    J --> K["StoreStore+StoreLoad屏障"]
    K --> L["刷回主存"]

    subgraph M["供应链高并发优化方案"]
        N1["分段锁<br/>分散SKU竞争"]
        N2["AtomicLong<br/>无锁CAS"]
        N3["Redis Lua<br/>前移扣减"]
    end
...
<img src="/interview-2026/images/diagram_pdd-scm_pdd-scm-001.svg" alt="【拼多多供应链】synchronized 的锁升级过程？JMM 怎么保证可见性？ - 核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
