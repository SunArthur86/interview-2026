...
```mermaid
flowchart TD
    A["线上触发OOM告警"] --> B["自动重启并恢复服务"]
    B --> C["导出 Dump (hprof)"]
    C --> D["MAT 内存分析"]
    D --> E["查看 Dominator Tree"]
    E --> F["锁定 Retained Heap 最大对象"]
    F --> G["排查 Path to GC Roots"]
    G --> H{"是否被 static 持有?"}
    H -- "是" --> I["确认: 缓存无上限或引用未释放"]
    I --> J["代码修复: 精简对象或改用 Redis"]
    J --> K["压力验证与上线监控"]
    H -- "否" --> L["排查瞬时大对象或内存泄漏"]
    L --> K
```
...
<img src="/interview-2026/images/diagram_pdd-scm_pdd-scm-002.svg" alt="【拼多多供应链】线上 OOM 怎么排查？供应链系统最常见的 OOM 场景？ - 核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
