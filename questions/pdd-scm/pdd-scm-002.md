...
- 内存泄漏
feynman:
  essence: JVM 内存分堆/非堆/直接内存，OOM 按区域分 5 类；排查靠"分类型→dump→MAT 支配树→GC Root 引用链→代码"五步，供应链最常见的
    OOM 是大批量商品数据一次性加载。
...
flowchart TD
    ...
    class Path info
    class Redis start
    class Retained process
    class Roots decision
    class Tree special
    class hprof error
    class static info
    class to start
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:12<br/>OOM 五类型"]:::concept
    N3["1:04<br/>能力/参数"]:::deep
    N4["1:56<br/>MAT 三件套"]:::deep
  end
  subgraph Outro["🎯 收尾"]
