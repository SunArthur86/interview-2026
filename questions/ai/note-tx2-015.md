...
- 算法题
feynman:
  essence: 滑动窗口实现长对话自动摘要截断，控制总 token 上限——维护一个窗口，新消息进来如果总 token 超限，就把最旧的几条摘要成一条再保留。用
    deque 维护消息队列，tokenizer 计算 token 数，超限时摘除最旧的 + LLM 摘要 + 插入摘要。关键是"边淘汰边摘要"，保证不丢关键信息又能控
    token。
