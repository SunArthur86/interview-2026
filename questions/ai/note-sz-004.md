...
flowchart TD
    ...
    D --> E["小表: 扩容N倍<br/>整体走两阶段Join"]
    B -- "GroupBy高频Key" --> F["阶段1: 加随机前缀局部聚合"]
    F --> G["阶段2: 去除前缀全局聚合"]
    B -- "大表Join大表(有极小表)" --> H["MapJoin广播小表<br/>避免Shuffle产生倾斜"]
    C --> I["缓解99%任务等待现象"]
    E --> I
    G --> I
    H --> I
...
flowchart LR
    ...
    subgraph Intro["🎥 引入"]
        N0["数据倾斜怎么处理？<br/>0:00"]:::intro
    end

    subgraph Core["📖 核心讲解"]
        N1["核心概念图<br/>0:20"]:::core
        N2["倾斜本质示意图<br/>0:50"]:::deep
        N3["三类常见key示意图<br/>1:30"]:::deep
    end
