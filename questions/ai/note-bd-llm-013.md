...
flowchart LR
    ...
    subgraph 异构召回
    A --> B[稀疏检索 BM25<br/>精确关键词匹配]
    A --> V[稠密检索 向量<br/>深层语义理解]
    end

    B --> R1[文档排名列表 1]
    V --> R2[文档排名列表 2]

    R1 --> F[RRF倒数排名融合<br/>公式: 1 / k + rank]
    R2 --> F
...
flowchart LR
    ...
    subgraph Intro["🎥 引入"]
        N0["稀疏检索和稠密检索的结果如何做融合…<br/>0:00"]:::intro
    end

    subgraph Core["📖 核心讲解"]
        N1["核心概念图<br/>0:20"]:::core
        N2["BM25示意图<br/>0:50"]:::deep
        N3["向量(稠密示意图<br/>1:30"]:::deep
    end
