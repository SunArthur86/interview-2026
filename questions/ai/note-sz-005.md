...
- 窗口函数
feynman:
  essence: 窗口函数能在不聚合行的情况下对"一组相关行"做计算，保留明细同时算聚合/排名/偏移。四类：①聚合类(SUM/AVG/COUNT OVER)②排名类(ROW_NUMBER/RANK/DENSE_RANK)③偏移类(LAG/LEAD)④分布类(PERCENTILE/NTILE)。出行场景实例：司机收入排名用ROW_NUMBER，连续登录判断用ROW_NUMBER差值，环比增长用LAG，累计流水用SUM
    OVER累加。
...
graph TD
    ...
    class CUME_DIST special
    class D error
    class D1 info
    class DENSE_RANK start
    class E process
    class E1 decision
    class FRAME special
    class LAG error
    ...
    class OVER process
    class RANK decision
    class ROW_NUMBER special
    class SQL error
    class SUM info
    class br start
    A["SQL 开窗函数 OVER"] --> B["聚合类<br/>保留明细算聚合"]
    A --> C["排名类<br/>排序与TopN"]
    A --> D["偏移类<br/>时间环比同比"]
    A --> E["分布类<br/>数据分桶百分位"]

    B --> B1["SUM/AVG/COUNT<br/>配合 FRAME 范围"]
    C --> C1["ROW_NUMBER<br/>连续唯一不重复"]
    C --> C2["RANK<br/>并列重复且跳号"]
    C --> C3["DENSE_RANK<br/>并列重复不跳号"]
    D --> D1["LAG / LEAD<br/>取前N行/后N行"]
