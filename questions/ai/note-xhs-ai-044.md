...
first_principle:
  ...
  derivation: LLM生成时每一步都在概率分布上采样。即使检索到的上下文包含正确信息，采样时仍有一定概率生成与上下文矛盾的内容（特别是当模型先验知识与上下文冲突时）。因此需要：1)
    Prompt层面降低幻觉概率；2) 生成后检测并拦截矛盾内容；3) 提供溯源链路方便核查
...
flowchart LR
    ...
    subgraph Intro["🎥 引入"]
        N0["RAG缓解幻觉的边界在哪里？三层管…<br/>0:00"]:::intro
    end

    subgraph Core["📖 核心讲解"]
        N1["核心概念图<br/>0:20"]:::core
        N2["RAG缓解幻觉的边界示意图<br/>0:50"]:::deep
    end
