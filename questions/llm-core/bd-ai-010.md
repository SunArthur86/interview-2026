...
flowchart TD
    ...
    class D special
    class E error
    class Engineering info
    class F start
    class G process
    class H decision
    class Prompt special
    A[模糊用户意图] --> B[Prompt Engineering]
    B --> C[精准指令引导概率分布]
    C --> D[角色/任务/上下文/格式/约束]
    D --> E[激活模型对应知识区]
    E --> F[确定的高质量输出]
...
**30 秒电梯演讲：** 通过精准指令引导概率分布，将模糊意图转化为确定输出。——打个比方，像是对着只会听指令的机器人说话，指令越具体，它动作越精准。

**展开框架：**
1. **核心目标** — 翻译意图为模型易理解形式，压缩概率分布，提高输出确定性
2. **三大策略** — System Prompt定角色/风格，Few-shot对齐格式/模式，CoT解决复杂推理
3. **效果差异** — 好Prompt概率分布集中(尖峰)，差Prompt分布平坦(随机)

**收尾：** 以上三点都能配合实战聊。我可以展开任一要点，比如「Zero-shot vs Few-shot怎么选？——简单任务Zero-shot够，复杂格式用Few-shot」这类追问您感兴趣吗？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡 | "【字节面经】Prompt Engineering的核心目标是什么，30 秒讲清楚。" | 开场钩子 |
