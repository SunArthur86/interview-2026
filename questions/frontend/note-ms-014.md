...
flowchart TD
    ...
    class C5 info
    class C6 start
    class ContextStore process
    class Core decision
    class EventBus special
    class LifecycleMgr error
    class P1 info
    class P2 start
    class P3 process
    class P4 decision
    class Plugin special
    class PluginRegistry error
    class Plugins info
    class SH1 start
    class SH2 process
    class SH3 decision
    class SH4 special
    class Shell error
    class SkillRouter info
    class Skills start
    class UIRenderer process
    class br decision
    class chart special
    class draft error
    class export info
    class generate start
    class query process
    class rewrite decision
    class summarize special
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["【月之暗面面经】如果产品要加更多 Agent，前端怎样保持扩…"]:::intro
    end

    subgraph Core["讲解"]
        B["核心思想微内核架构：保持核心极简稳定，Agent 均…"]:::core
        C["标准接入三件套：插件提供标准 Schema，通过注册…"]:::deep
    end
