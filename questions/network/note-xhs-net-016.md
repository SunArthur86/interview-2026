...
<img src="/interview-2026/images/diagram_network_note-xhs-net-016.svg" alt="WebSocket 主要是用来做什么的？如何设计实现一个 WebSocket 服务？（小红书Java一面）" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
## 结构化回答
...
sequenceDiagram
    ...
    C->>S: HTTP GET 握手 Upgrade: websocket
    S-->>C: HTTP 101 Switching Protocols 同意升级
    rect rgb(240, 248, 255)
    Note over C,S: TCP连接复用，切换为WebSocket全双工通信
    loop 长连接通信
        C->>S: 掩码数据帧 opcode 文本或二进制
        S-->>C: 数据帧 无掩码，降低CPU开销
    end
    loop 心跳保活
        C->>S: Ping帧 心跳请求
