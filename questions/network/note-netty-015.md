...
- Mina
feynman:
  essence: Netty 是 Java 高性能网络通信的事实标准——几乎所有需要"高性能、低时延通信"的中间件都建立在它之上。三大主战场：①高性能中间件的通信底座（MQ/RPC/ESB）；②公有/私有协议栈的基础框架（如
    WebSocket）；③各领域应用的模块间数据分发（大数据、游戏）。它的设计理念继承自 Mina，由同一作者主导。
  analogy: Netty 像"互联网基建的水管网络"——你平时用着各种 App（Dubbo、Spark、ES、RocketMQ），表面上看不到 Netty，但它们的底层通信都流淌在
    Netty 这套水管里。就像你拧开水龙头有水，但你不会看到埋在地下的水管——Netty 就是那套埋在地下的高性能水管。
  key_points:
