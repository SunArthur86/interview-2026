...
flowchart TD
    ...
    class FILE error
    class File info
    class Filesystem start
    class Filter process
    class Injection decision
    class Input special
    class Intent error
    class Network info
    ...
    class Whitelist special
    class applicable error
    class br info
    class only start
    class where process
    USER["User / Attacker"]
...
flowchart LR
    ...
    subgraph Intro["🎥 引入"]
        N0["Agent 权限控制<br/>0:00"]:::intro
    end

    subgraph Core["📖 核心讲解"]
        N1["最小权限 + 沙箱架构<br/>0:30"]:::core
    end

    subgraph Practice["🔧 实战"]
        N2["分级审批 L0-L3<br/>1:00"]:::practice
    end

    subgraph Wrap["🎬 收尾"]
        N3["SQL 模板 + 审计追踪<br/>1:30"]:::wrap
    end
