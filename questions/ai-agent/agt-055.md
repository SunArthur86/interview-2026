...
flowchart TB
    ...
    class LM info
    class LMHead start
    class LayerNorm process
    class Logits decision
    class Next special
    class Norm error
    class Parallel info
    class Pos start
    class Prefill process
    class Sample decision
    class Self special
    class Serial error
    class Token info
...
flowchart LR
    ...
    subgraph Intro["🎥 引入"]
        N0["大模型推理<br/>0:00"]:::intro
    end

    subgraph Core["📖 核心讲解"]
        N1["Prefill 并行计算动画<br/>0:30"]:::core
        N2["Decode 串行 + 读 KV Cache 动画<br/>1:15"]:::deep
    end

    subgraph Practice["🔧 实战"]
        N3["KV Cache 显存随长度增长曲线<br/>2:00"]:::practice
    end

    subgraph Wrap["🎬 收尾"]
        N4["Top-p 采样示意图<br/>2:40"]:::wrap
    end
