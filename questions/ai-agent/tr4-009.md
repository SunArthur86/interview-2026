...
flowchart TB
    ...
    class Least error
    class Log info
    class Permission start
    class Resource process
    class Risk decision
    class Shell special
    class Timeout error
    class Wasm info
    class WebAssembly start
...
flowchart LR
    ...
    subgraph Intro["🎥 引入"]
        N0["Agent 工具沙箱<br/>0:00"]:::intro
    end

    subgraph Core["📖 核心讲解"]
        N1["通风橱防爆手套箱类比<br/>0:15"]:::core
        N2["沙箱五层防御图<br/>0:40"]:::deep
        N3["AST 解析防混淆示意图<br/>1:10"]:::deep
    end

    subgraph Practice["🔧 实战"]
        N4["输出脱敏防外带案例<br/>1:35"]:::practice
    end

    subgraph Wrap["🎬 收尾"]
        N5["口诀：隔离限权过滤，输入输出都要查，防供应链投毒<br/>1:55"]:::wrap
    end
