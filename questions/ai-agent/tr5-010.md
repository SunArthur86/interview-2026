...
flowchart TB
    ...
    class Curve start
    class Decay process
    class Ebbinghaus decision
    class Effect special
    class Emotion error
    class Factors info
    class Impl start
    class Importance process
    class Mem decision
    class R special
    class Repetition error
    class S info
    class Score start
    class br process
    class e decision
...
flowchart LR
    ...
    subgraph Intro["🎥 引入"]
        N0["Agent 遗忘曲线<br/>0:00"]:::intro
    end

    subgraph Core["📖 核心讲解"]
        N1["大脑自然遗忘类比<br/>0:15"]:::core
        N2["有效分值计算公式<br/>0:40"]:::core
        N3["降权非删除示意图<br/>1:10"]:::deep
    end

    subgraph Practice["🔧 实战"]
        N4["永久标记+保护期示意<br/>1:35"]:::practice
    end

    subgraph Wrap["🎬 收尾"]
        N5["口诀：时间降权重，永久标记护核心，保护期防误杀。<br/>1:55"]:::wrap
    end
