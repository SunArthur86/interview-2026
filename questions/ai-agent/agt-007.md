...
flowchart TB
    ...
    class ACL start
    class Agent process
    class Challenges decision
    class Cost special
    class Eval error
    class Hall info
    class Inject start
    class Latency process
    class Loop2 decision
    class Mitigate special
    class Mitigate2 error
    class Multi2 info
    class Prompt start
    class Reliability process
...
flowchart LR
    ...
    subgraph Intro["🎥 引入"]
        N0["Agent 的五大挑战<br/>0:00"]:::intro
    end

    subgraph Core["📖 核心讲解"]
        N1["幻觉示意图：模型编造 API 参数<br/>0:25"]:::core
        N2["Prompt 注入攻击示意图<br/>0:55"]:::deep
        N3["成本优化三招对比表<br/>1:30"]:::deep
    end

    subgraph Practice["🔧 实战"]
        N4["Thought/Action/Observation …<br/>2:10"]:::practice
    end

    subgraph Wrap["🎬 收尾"]
        N5["五维挑战总结卡<br/>2:50"]:::wrap
    end
