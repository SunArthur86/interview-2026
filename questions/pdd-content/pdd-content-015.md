...
flowchart TD
    ...
    B -->|"置信度 > 0.95"| D["自动通过上架"]
    B -->|"置信度 < 0.5"| E["自动拒绝违规"]
    B -->|"0.5 - 0.95 疑难"| F["人工审核复核"]
    F --> D
    F --> E
    D --> G["反作弊检测<br/>SimHash/设备指纹"]
    G --> H["触发评分更新"]
    H --> I["Redis Lua原子聚合<br/>hincrby count/score"]
    I --> J["定时全量校准DB"]
    G --> K["行家社区权威加权"]
    K --> L["排序展示/个性化推送"]
```
...
<img src="/interview-2026/images/diagram_pdd-content_pdd-content-015.svg" alt="【拼多多内容】评价业务怎么设计（含行家社区）？ - 核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
