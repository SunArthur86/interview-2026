...
flowchart TD
    ...
    subgraph Multi-Level Cache
        ...
        C -- 未命中 --> D[L2: Redis分布式缓存<br/>分担60%请求流量]
        D -- 未命中 --> E[L3: MySQL数据库<br/>最终兜底]
    end
    E --> F[读写分离/读从库]
    F --> G[回写L1与L2缓存]
    
    H[编辑评价写请求] --> I[写入MySQL主库]
    I --> J[删除Redis缓存<br/>防抖合并避免击穿]
    J --> K[广播清理各实例L1缓存]
    
    L[突发写高峰] --> M[Kafka异步队列削峰]
    M --> N[独立线程池异步消费]
```
...
<img src="/interview-2026/images/diagram_pdd-content_pdd-content-026.svg" alt="【拼多多内容】高并发四件套（池化/缓存/扩容/异步队列）？ - 核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
