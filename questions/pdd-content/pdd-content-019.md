...
flowchart TD
    ...
    class H special
    class I error
    class IdleStateHandler info
    class J start
    class K process
    class LengthFieldBasedFrameDecoder decision
    class Netty special
    class PooledByteBufAllocator error
    class Worker info
    class br start
    A[百万直播观众] -- WebSocket长连接 --> B[直播弹幕网关]
    subgraph Netty Gateway
        C[Boss EventLoop组] -- accept接收连接 --> D[Worker EventLoop组]
        D -- epoll事件通知 --> E[IO读写处理]
        E --> F[LengthFieldBasedFrameDecoder<br/>解决半包粘包]
        F --> G[业务处理Handler]
        G -- 慢业务不阻塞IO --> H[独立业务线程池<br/>风控/敏感词匹配]
    end
    H --> I[Kafka异步队列<br/>弹幕分发推送]

    J[JVM堆外内存监控] -. PooledByteBufAllocator .- E
    K[IdleStateHandler] -. 心跳检测保活 .- D
```
