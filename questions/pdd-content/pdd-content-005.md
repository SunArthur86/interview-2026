...
flowchart TD
    subgraph OOM触发与现场保留
        A1["内容服务发生OOM"] --> A2["JVM自动触发HeapDump"]
        A2 --> A3["MAT解析Dump文件"]
    end

    subgraph 根因定位链路
        B1["查看Dominator Tree<br/>按Retained Size排序"] --> B2["右键Path to GC Roots<br/>排除Weak/Soft引用"]
        B2 --> C1["Map无界增长<br/>换Caffeine"]
        B2 --> C2["CGLIB代理类膨胀<br/>复用代理对象"]
        B2 --> C3["Netty DirectByteBuf泄漏<br/>补Release"]
    end
```
...
<img src="/interview-2026/images/diagram_pdd-content_pdd-content-005.svg" alt="【拼多多内容】OOM 类型与排查（内容服务实战）？ - 核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
