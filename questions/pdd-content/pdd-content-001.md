...
flowchart TD
    ...
    P -- 评价写入 --> B1[评价独立线程池]
    B1 --> O1[操作DB与ES同步]
    P -- Feed流写入 --> B2[Feed独立线程池]
    B2 --> O2[Redis推拉扩散发件箱]

    subgraph 线程池核心执行序
      C1[核心线程满载] --> C2[进入任务缓冲队列]
      C2 --> C3[扩容至最大线程数]
      C3 --> C4[触发拒绝策略降级]
    end
...
<img src="/interview-2026/images/diagram_pdd-content_pdd-content-001.svg" alt="【拼多多内容】评价/Feed 写入线程池怎么设计与隔离？ - 核心知识点图" style="max-width:100%;height:auto;border:1px solid var(--border);border-radius:8px;margin:1em 0;" />
