...
flowchart TD
    ...
    class Agent process
    class App decision
    class ByteBuddy special
    class Header error
    class Inject info
    class Instrumentation start
    class Java process
    class ThreadLocal decision
    class Trace special
    class br error
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["OpenTelemetry在Java Agent模式下是如何…"]:::intro
    end

    subgraph Core["讲解"]
        B["Agent基于Java Instrumentatio…"]:::core
        C["无侵入原理是在核心组件方法入口与出口，强行注入提取与…"]:::deep
    end
