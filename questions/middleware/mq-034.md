...
flowchart TD
    ...
    class Cmd start
    class ControllerEvent process
    class Event decision
    class LeaderAndIsr special
    class LeaderISR error
    class Queue info
    class Single start
    class Stop process
    class StopReplica decision
    class Trigger special
    class Update error
    class UpdateMetadata info
    class br start
...
flowchart LR
    ...
    subgraph Core["讲解"]
        B["口诀定框架：ZK监听触发、事件队列解耦、单线程串行、…"]:::core
        C["因为事件放入内存阻塞队列，由单线程串行处理，所以保证…"]:::deep
    end
