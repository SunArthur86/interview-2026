...
flowchart TB
    ...
    class B start
    class Bucket process
    class Current decision
    class DelayQueue special
    class Exp error
    class Higher info
    ...
    class Timing decision
    class TimingWheel special
    class br error
    class delay info
    class l1 start
    class l2 process
...
flowchart LR
    ...
    subgraph Core["讲解"]
        B["层次化结构：采用多级时间轮，类似于时钟的秒分时针，长…"]:::core
        C["解决空推进：因为把「时间槽（TimerTaskLis…"]:::deep
    end
