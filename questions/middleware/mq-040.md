...
flowchart TD
    ...
    class Acceptor start
    class Client process
    class Conn decision
    class Handler special
    class M error
    class N info
    class Processor start
    class Queue process
    class RespQueue decision
    class br special
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["Kafka Broker 网络通信模型"]:::intro
    end

    subgraph Core["讲解"]
        B["三层架构：1个Acceptor建连 -》 N个Pro…"]:::core
        C["分工边界：Acceptor只接连接不读写，Proce…"]:::deep
    end
