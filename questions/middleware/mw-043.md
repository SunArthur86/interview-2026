...
flowchart TD
    ...
    class Analyze start
    class Dict process
    class Dictionary decision
    class Doc special
    class DocIDs error
    class Index info
    class List start
    class Posting process
    class Term decision
    class br special
    Doc[文档] --> Analyze[分词]
    Analyze --> Term[Term词条]
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["为什么选 Elasticsearch 做搜索而不是 MySQ…"]:::intro
    end

    subgraph Core["讲解"]
        B["对比MySQL：因为LIKE%无法走索引会触发全表扫…"]:::core
        C["倒排索引本质：正向是ID找内容，倒排则是分词后的词条…"]:::deep
    end
