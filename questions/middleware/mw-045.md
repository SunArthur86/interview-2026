...
flowchart TD
    ...
    class Mandatory decision
    class ManualAck special
    class Persistent error
    class Producer info
    class br start
    class mandatory process
    class true decision
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["什么场景选 RabbitMQ 而不是 Kafka？Rabbi…"]:::intro
    end

    subgraph Core["讲解"]
        B["选型场景：因为RabbitMQ具备丰富的Exchan…"]:::core
        C["可靠链路三步走：生产者确认机制（Confirm） +…"]:::deep
    end
