...
flowchart TD
    ...
    class Istio process
    class K8s decision
    class NewVersion special
    class Route error
    class Traffic info
    class VS start
    class VirtualService process
    class br decision
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["在微服务架构中，如何实现全链路灰度发布？请结合K8s和Ser…"]:::intro
    end

    subgraph Core["讲解"]
        B["核心痛点：全链路灰度旨在让特定流量（如Header标…"]:::core
        C["Mesh优势：Istio低侵入，业务零感知，而传统S…"]:::deep
    end
