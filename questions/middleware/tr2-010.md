...
flowchart TD
    ...
    class Backlog start
    class DBOpt process
    class Degrade decision
    class Diagnose special
    class Downstream error
    class Scale info
...
flowchart LR
    ...
    subgraph Intro["引入"]
        A["在处理 Kafka 消息积压时，如果仅仅增加消费者实例（Co…"]:::intro
    end

    subgraph Core["讲解"]
        B["盲目增加Consumer无效且引发Rebalance…"]:::core
        C["紧急处理方案：将原Topic消息转发至大分区临时To…"]:::deep
    end
