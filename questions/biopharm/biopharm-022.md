...
feynman:
  ...
  first_principle: 各模型 API 协议、参数、错误码、限流策略都不同，若业务直连各家，会写成 N 套适配、改一处动全身。LLM 接入工程化的本质是'把异构模型的差异收敛到统一
    SDK，让上层无感切换'，同时提供可靠性（错误处理/重试/fallback）和可观测性。
  key_points:
...
follow_up:
- 怎么统一不同模型的参数？——抽 common params（messages/temperature/max_tokens/stream）+ extra_args
  透传特有参数；能力差异用 capability flag 标注。
...
flowchart TD
    ...
    class Resilience error
    class SDK info
    class Stream start
    class Tool process
    class Vision decision
    subgraph Client[业务调用方]
        A1[RAG问答应用]
        A2[智能体编排应用]
    end

    subgraph SDK[统一大模型接入 SDK]
        B1[OpenAI兼容协议路由]
        B2[能力查询: Tool/Vision/Stream]
    end

    subgraph Adapter[供应商适配层]
        C1[OpenAI适配器]
        C2[Claude适配器]
        C3[通义千问适配器]
    end

    subgraph Resilience[高可用容灾中心]
        D1{"错误类型判定"}
        D2[指数退避重试]
        D3[携带幂等ID防重复扣费]
        D4[主备模型切换与规则降级]
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    ...
    N3["0:40<br/>错误分类"]:::concept
    N4["1:05<br/>重试策略"]:::deep
  end
  subgraph Outro["🎯 收尾"]
