...
feynman:
  ...
  first_principle: LLM 推理是显存和算力密集（自回归逐 token 生成），瓶颈在显存带宽和 KV Cache 占用。推理优化的本质是'提升
    GPU 利用率（批处理）、减少重复计算（KV Cache 复用）、降低显存占用（量化）'，把单位成本压下来。
  key_points:
...
first_principle:
  ...
  - GPU 利用率低（逐 token 等待）
  rebuild: 三招提效——vLLM（PagedAttention+连续批处理）提 GPU 利用率和吞吐、KV Cache 复用减重复计算、量化降显存多装请求，把单位
    token 成本压到最低。
follow_up:
...
flowchart TD
    ...
    class Prefix start
    class vLLM process
    A[多用户并发推理请求] --> B[vLLM 推理引擎]
    subgraph C [核心优化策略]
        direction LR
        C1[PagedAttention 显存分页]
        C2[Continuous Batching 动态拼批]
        C3[Prefix Cache 复用系统Prompt]
        C4[INT8/INT4 模型量化压缩]
    end
    B --> C
    C1 --> D[减少显存碎片/提升并发数]
...
flowchart LR
  ...
  subgraph Body["📚 讲解"]
    N2["0:15<br/>缓存"]:::concept
    N3["0:50<br/>KV Cache"]:::practice
    N4["1:20<br/>量化"]:::deep
    N5["1:50<br/>批处理"]:::concept
  end
  subgraph Outro["🎯 收尾"]
